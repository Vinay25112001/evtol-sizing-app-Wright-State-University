...
const TABS=["Overview","Mission","Wing & Aero","Propulsion","Battery","Performance","Stability","V-Tail","Convergence","Monte Carlo","Certification","Noise","Cost","Mission Builder","Weather & Atmos","OpenVSP","Community","Collaboration","V-n Diagram","Design Space","BEM Rotor","Reg Tracker","AI Assistant","Payload-Range","Hover Endurance","Acoustic Directivity","Load Path"];
const TABI=["⬛","🛫","✈️","🔧","🔋","📈","⚖️","🦋","🔄","🎲","📋","🔊","💰","🗺️","🌤️","🛩️","🌐","👥","📐","🎯","🔬","📜","🤖","📦","🔋","📡","🏗️"];
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"14px 18px 28px",background:SC.bg}}>
            ...
            {/* ──── TAB 23: PAYLOAD-RANGE DIAGRAM (INTERACTIVE) ──── */}
            {tab===23&&SR&&(()=>{
              // Sweep payload 0→maxPay, compute max range for each
              // Also sweep range 0→maxRange to show iso-lines
              const g0=9.81;
              const maxPay = SR.MTOW - SR.Wempty - 20;
              const Eto=SR.Eto, Eld=SR.Eld, Etot=SR.Etot;
              const Efl=Etot-Eto-Eld;

              // Primary boundary: payload vs max range
              const boundary = Array.from({length:80},(_,i)=>{
                const pay = maxPay*i/79;
                const Wavail = SR.MTOW - SR.Wempty - pay;
                if(Wavail<=0) return{payload:+pay.toFixed(0),range:0};
                const Eavail = Wavail*params.sedCell*params.etaBat/(1000*(1+params.socMin));
                const rng = Math.max(0,((Eavail-Eto-Eld)/Math.max(Efl,0.001))*params.range);
                return{payload:+pay.toFixed(0),range:+rng.toFixed(1)};
              }).filter(d=>d.range>=0);

              const maxRange = Math.max(...boundary.map(d=>d.range),1);

              // Competitor reference points (real-world UAM data)
              const competitors=[
                {name:"Joby S4",payload:250,range:240,col:"#f59e0b"},
                {name:"Archer Midnight",payload:272,range:97,col:"#3b82f6"},
                {name:"Lilium Jet",payload:200,range:186,col:"#8b5cf6"},
                {name:"Wisk Aero",payload:136,range:90,col:"#06d6a0"},
                {name:"Overair Butterfly",payload:375,range:100,col:"#ef4444"},
              ];

              // Iso-energy lines (battery weight fractions)
              const batFracs=[0.25,0.35,0.45];
              const isoLines = batFracs.map(frac=>{
                const pts=Array.from({length:40},(_,i)=>{
                  const pay=maxPay*i/39;
                  const Wb=SR.MTOW*frac;
                  const Eavail2=Wb*params.sedCell*params.etaBat/(1000*(1+params.socMin));
                  const rng=Math.max(0,((Eavail2-Eto-Eld)/Math.max(Efl,0.001))*params.range);
                  return{payload:+pay.toFixed(0),range:+Math.min(rng,maxRange*1.1).toFixed(1)};
                });
                return{frac,pts};
              });

              const designPoint={payload:params.payload,range:params.range};

              return(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:"14px 18px"}}>
                  <div style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",letterSpacing:"0.15em",marginBottom:2}}>MISSION ANALYSIS</div>
                  <div style={{fontSize:20,fontWeight:800,color:SC.amber}}>Payload-Range Diagram</div>
                  <div style={{fontSize:10,color:SC.muted,marginTop:2}}>Interactive trade space — boundary curve shows maximum range achievable at each payload. Design point shown in amber.</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                  <KPI label="Max Payload" value={maxPay.toFixed(0)} unit="kg" color={SC.green}/>
                  <KPI label="Design Range" value={params.range} unit="km" color={SC.blue}/>
                  <KPI label="Max Ferry Range" value={boundary[0]?.range?.toFixed(0)||"—"} unit="km" color={SC.teal} sub="zero payload"/>
                  <KPI label="Design Point" value={((params.payload/Math.max(maxPay,1))*100).toFixed(0)} unit="% max pay" color={SC.amber}/>
                </div>
                <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14}}>
                  <div style={{fontSize:10,fontWeight:700,color:SC.text,marginBottom:10,fontFamily:"'DM Mono',monospace"}}>PAYLOAD (kg) vs RANGE (km) — Boundary Curve + Competitor References</div>
                  <ResponsiveContainer width="100%" height={380}>
                    <ComposedChart margin={{top:10,right:30,left:10,bottom:30}}>
                      <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                      <XAxis type="number" dataKey="payload" domain={[0,Math.ceil(maxPay/50)*50]}
                        tick={{fontSize:10,fill:SC.muted}}
                        label={{value:"Payload (kg)",position:"insideBottom",offset:-12,fontSize:12,fill:SC.muted}}/>
                      <YAxis type="number" dataKey="range" domain={[0,Math.ceil(maxRange/50)*50]}
                        tick={{fontSize:10,fill:SC.muted}}
                        label={{value:"Range (km)",angle:-90,position:"insideLeft",fontSize:12,fill:SC.muted}}/>
                      <Tooltip {...TTP} formatter={(v,n)=>[`${v}${n==="range"?" km":" kg"}`,n]}/>
                      <Legend iconSize={10} wrapperStyle={{fontSize:11,color:SC.muted,paddingTop:8}}/>
                      {/* Iso-battery lines */}
                      {isoLines.map(({frac,pts})=>(
                        <Line key={frac} data={pts} type="monotone" dataKey="range"
                          stroke={SC.dim} strokeWidth={1} strokeDasharray="4 3" dot={false}
                          name={`Bat frac ${(frac*100).toFixed(0)}%`}/>
                      ))}
                      {/* Main boundary — filled area */}
                      <Area data={boundary} type="monotone" dataKey="range"
                        stroke={SC.teal} strokeWidth={2.5}
                        fill={SC.teal+"22"} dot={false} name="Design envelope"/>
                      {/* Competitor scatter */}
                      {competitors.map(c=>(
                        <Scatter key={c.name} data={[c]} dataKey="range" name={c.name} fill={c.col}
                          shape={(props)=>{
                            const {cx,cy}=props;
                            return<g>
                              <circle cx={cx} cy={cy} r={7} fill={c.col} opacity={0.85}/>
                              <text x={cx+10} y={cy+4} fontSize={9} fill={c.col} fontFamily="DM Mono,monospace">{c.name}</text>
                            </g>;
                          }}/>
                      ))}
                      {/* Design point */}
                      <ReferenceLine x={designPoint.payload} stroke={SC.amber} strokeDasharray="5 3"
                        label={{value:`Design ${designPoint.payload} kg`,fill:SC.amber,fontSize:9,position:"top"}}/>
                      <ReferenceLine y={designPoint.range} stroke={SC.amber} strokeDasharray="5 3"
                        label={{value:`${designPoint.range} km`,fill:SC.amber,fontSize:9,position:"right"}}/>
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  <Panel title="Range-Payload Data Table">
                    <div style={{maxHeight:220,overflowY:"auto"}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"'DM Mono',monospace"}}>
                        <thead><tr style={{borderBottom:`1px solid ${SC.border}`}}>
                          {["Payload (kg)","Max Range (km)","Bat Used (kWh)","% of Max Pay"].map(h=>(
                            <th key={h} style={{padding:"3px 6px",textAlign:"left",fontSize:8,color:SC.muted,textTransform:"uppercase"}}>{h}</th>
                          ))}</tr></thead>
                        <tbody>{boundary.filter((_,i)=>i%8===0).map((d,i)=>{
                          const pct=(d.payload/Math.max(maxPay,1)*100).toFixed(0);
                          const Wavail2=SR.MTOW-SR.Wempty-d.payload;
                          const Eavail2=Wavail2>0?Wavail2*params.sedCell*params.etaBat/(1000*(1+params.socMin)):0;
                          const Eused=Math.min(Eavail2,Etot);
                          return<tr key={i} style={{background:i%2===0?SC.bg:"transparent",borderBottom:`1px solid ${SC.border}22`}}>
                            <td style={{padding:"3px 6px",color:SC.amber,fontWeight:700}}>{d.payload}</td>
                            <td style={{padding:"3px 6px",color:d.range>0?SC.teal:SC.red}}>{d.range}</td>
                            <td style={{padding:"3px 6px",color:SC.text}}>{Eused.toFixed(1)}</td>
                            <td style={{padding:"3px 6px",color:SC.muted}}>{pct}%</td>
                          </tr>;
                        })}</tbody>
                      </table>
                    </div>
                  ...
                  <Panel title="Competitor Comparison">
                    {competitors.map((c,i)=>{
                      const inEnvelope = boundary.some(d=>d.payload>=c.payload&&d.range>=c.range);
                      return<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                        padding:"6px 8px",marginBottom:4,background:SC.bg,borderRadius:4,
                        border:`1px solid ${c.col}33`,borderLeft:`3px solid ${c.col}`}}>
                        <div>
                          <div style={{fontSize:11,fontWeight:700,color:c.col,fontFamily:"'DM Mono',monospace"}}>{c.name}</div>
                          <div style={{fontSize:9,color:SC.muted}}>{c.payload} kg · {c.range} km</div>
                        </div>
                        <div style={{fontSize:9,fontWeight:700,color:inEnvelope?SC.green:SC.red,fontFamily:"'DM Mono',monospace"}}>
                          {inEnvelope?"✓ IN ENVELOPE":"✗ EXCEEDS"}
                        </div>
                      </div>;
                    })}
                  </Panel>
            ...
            {/* ──── TAB 24: HOVER ENDURANCE MAP (2D CONTOUR) ──── */}
            {tab===24&&SR&&(()=>{
              const g0=9.81, rhoMSL=1.225;
              const MTOW=SR.MTOW, Wempty=SR.Wempty;
              const nRot=params.nPropHover;
              const etaHov=params.etaHov||0.70;
              const etaBat=params.etaBat||0.90;
              const socMin=params.socMin||0.19;
              const sedCell=params.sedCell||300;

              // Grid: battery capacity (x: Wbat 100→600 kg) vs rotor diameter (y: 2→5m)
              const batRange=Array.from({length:12},(_,i)=>+(100+i*45).toFixed(0));     // 100→595 kg
              const dRotRange=Array.from({length:10},(_,i)=>+(2.0+i*0.35).toFixed(2));  // 2.0→5.15m

              const calcEndurance=(Wbat,Drot)=>{
                const Rrot=Drot/2;
                const Adisk=Math.PI*Rrot**2;
                const W=MTOW*g0;
                const T_r=W/nRot;
                const DL=T_r/Adisk;
                const Phov_kW=(W/etaHov)*Math.sqrt(DL/(2*rhoMSL))/1000;
                const Eavail=Wbat*sedCell*etaBat/(1000*(1+socMin));
                const enduranceHr=Eavail/Math.max(Phov_kW,0.01);
                return+Math.min(enduranceHr*60,240).toFixed(1); // minutes, cap 240
              };

              // Build grid
              const grid=dRotRange.map(d=>({
                dRot:d,
                cells:batRange.map(wb=>({wb,end:calcEndurance(wb,d)}))
              }));

              // Find min/max for color scale
              const allEnds=grid.flatMap(r=>r.cells.map(c=>c.end));
              const minEnd=Math.min(...allEnds), maxEnd=Math.max(...allEnds);

              const endColor=(v)=>{
                const t=(v-minEnd)/Math.max(maxEnd-minEnd,1);
                if(t<0.25) return`hsl(240,80%,${30+t*80}%)`;
                if(t<0.5)  return`hsl(${240-t*240},80%,50%)`;
                if(t<0.75) return`hsl(${60-(t-0.5)*120},80%,50%)`;
                return`hsl(0,80%,${60+(t-0.75)*40}%)`;
              };

              const curWbat=SR.Wbat, curDrot=SR.Drotor;
              const curEnd=calcEndurance(curWbat,curDrot);

              // Line chart: endurance vs bat capacity at current rotor size
              const endVsBat=batRange.map(wb=>({wb,end:calcEndurance(wb,curDrot)}));
              // Line chart: endurance vs rotor diameter at current bat
              const endVsDrot=dRotRange.map(d=>({dRot:d,end:calcEndurance(curWbat,d)}));

              return(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:"14px 18px"}}>
                  <div style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",letterSpacing:"0.15em",marginBottom:2}}>PROPULSION ANALYSIS</div>
                  <div style={{fontSize:20,fontWeight:800,color:SC.teal}}>Hover Endurance Map</div>
                  <div style={{fontSize:10,color:SC.muted,marginTop:2}}>2D trade space: battery mass (x) × rotor diameter (y) → hover endurance (z). Current design shown with ✦.</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                  <KPI label="Current Endurance" value={curEnd.toFixed(1)} unit="min" color={SC.green}/>
                  <KPI label="Battery Mass" value={curWbat.toFixed(0)} unit="kg" color={SC.amber}/>
                  <KPI label="Rotor Diameter" value={curDrot} unit="m" color={SC.blue}/>
                  <KPI label="Hover Power" value={SR.Phov} unit="kW" color={SC.teal}/>
                </div>
                {/* 2D Heat Map */}
                <Panel title="Endurance Heat Map — Rotor Diameter (m) vs Battery Mass (kg)">
                  <div style={{overflowX:"auto"}}>
                    <div style={{display:"grid",gridTemplateColumns:`60px repeat(${batRange.length},1fr)`,gap:2,minWidth:700,fontFamily:"'DM Mono',monospace"}}>
                      {/* Header row */}
                      <div style={{fontSize:8,color:SC.muted,padding:"2px 4px",display:"flex",alignItems:"flex-end"}}>D (m) →</div>
                      {batRange.map(wb=>(
                        <div key={wb} style={{fontSize:8,color:SC.muted,textAlign:"center",padding:"2px 0"}}>{wb}</div>
                      ))}
                      {/* Data rows */}
                      {grid.map(({dRot,cells})=>(
                        <>{
                          [
                            <div key={`lbl-${dRot}`} style={{fontSize:9,color:SC.muted,display:"flex",alignItems:"center",
                              padding:"2px 4px",fontWeight:dRot===curDrot||Math.abs(dRot-curDrot)<0.18?700:400,
                              color:Math.abs(dRot-curDrot)<0.18?SC.blue:SC.muted}}>{dRot}</div>,
                            ...cells.map(({wb,end})=>{
                              const isCur=Math.abs(wb-curWbat)<25&&Math.abs(dRot-curDrot)<0.18;
                              return<div key={wb} style={{
                                background:endColor(end),
                                borderRadius:3,padding:"4px 2px",textAlign:"center",
                                fontSize:8,fontWeight:isCur?800:400,
                                color:isCur?"#fff":end>60?"#fff":"rgba(255,255,255,0.7)",
                                border:isCur?`2px solid ${SC.amber}`:"none",
                                position:"relative"
                              }}>
                                {isCur?"✦ ":""}{end.toFixed(0)}
                              </div>;
                      ...
                      {/* Battery label */}
                      <div style={{gridColumn:`1 / ${batRange.length+2}`,fontSize:8,color:SC.muted,
                        textAlign:"center",padding:"6px 0",fontFamily:"'DM Mono',monospace"}}>
                        Battery Mass (kg) — values in minutes of hover endurance
                      </div>
                    </div>
                  </div>
                  {/* Color scale legend */}
                  <div style={{display:"flex",alignItems:"center",gap:8,marginTop:10,paddingTop:8,borderTop:`1px solid ${SC.border}`}}>
                    <span style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace"}}>{minEnd.toFixed(0)} min</span>
                    <div style={{flex:1,height:10,borderRadius:4,background:"linear-gradient(to right,hsl(240,80%,30%),hsl(120,80%,50%),hsl(60,80%,50%),hsl(0,80%,70%))"}}/>
                    <span style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace"}}>{maxEnd.toFixed(0)} min</span>
                  </div>
                </Panel>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  <Panel title={`Endurance vs Battery Mass (D=${curDrot}m fixed)`}>
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={endVsBat} margin={{top:5,right:20,left:5,bottom:20}}>
                        <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                        <XAxis dataKey="wb" tick={{fontSize:10,fill:SC.muted}}
                          label={{value:"Battery mass (kg)",position:"insideBottom",offset:-10,fontSize:11,fill:SC.muted}}/>
                        <YAxis tick={{fontSize:10,fill:SC.muted}}
                          label={{value:"Endurance (min)",angle:-90,position:"insideLeft",fontSize:11,fill:SC.muted}}/>
                        <Tooltip {...TTP} formatter={v=>[`${v} min`,"Endurance"]}/>
                        <ReferenceLine x={curWbat} stroke={SC.amber} strokeDasharray="4 3"
                          label={{value:"Current",fill:SC.amber,fontSize:9}}/>
                        <Line type="monotone" dataKey="end" stroke={SC.teal} strokeWidth={2.5} dot={false} name="Endurance (min)"/>
                      </LineChart>
                    </ResponsiveContainer>
                  </Panel>
                  <Panel title={`Endurance vs Rotor Diameter (Wbat=${curWbat.toFixed(0)}kg fixed)`}>
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={endVsDrot} margin={{top:5,right:20,left:5,bottom:20}}>
                        <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                        <XAxis dataKey="dRot" tick={{fontSize:10,fill:SC.muted}}
                          label={{value:"Rotor diameter (m)",position:"insideBottom",offset:-10,fontSize:11,fill:SC.muted}}/>
                        <YAxis tick={{fontSize:10,fill:SC.muted}}
                          label={{value:"Endurance (min)",angle:-90,position:"insideLeft",fontSize:11,fill:SC.muted}}/>
                        <Tooltip {...TTP} formatter={v=>[`${v} min`,"Endurance"]}/>
                        <ReferenceLine x={curDrot} stroke={SC.amber} strokeDasharray="4 3"
                          label={{value:"Current",fill:SC.amber,fontSize:9}}/>
                        <Line type="monotone" dataKey="end" stroke={SC.blue} strokeWidth={2.5} dot={false} name="Endurance (min)"/>
                      </LineChart>
                    </ResponsiveContainer>
                  </Panel>
                </div>
            ...
            {/* ──── TAB 25: ROTOR ACOUSTICS DIRECTIVITY PLOT ──── */}
            {tab===25&&SR&&(()=>{
              const dBA_1m=SR.dBA_1m;
              const r_ref=50; // reference propagation distance

              // Gutin dipole directivity: D(θ) = |sin θ| for thrust-axis dipole
              // Monopole (broadband): D(θ) = 1 (omnidirectional)
              // Combined: SPL_dir(θ) = SPL_1m - 20·log10(r) + 10·log10(D(θ)+ε)
              // θ = 0 = directly above rotor (thrust axis), 90 = in-plane (loudest)
              const N_pts=72; // every 5 degrees
              const directivityData=Array.from({length:N_pts},(_,i)=>{
                const theta_deg=i*(360/N_pts);
                const theta_rad=theta_deg*Math.PI/180;
                // Dipole directivity (loading noise dominates hover) — |sin θ|
                const D_dipole=Math.abs(Math.sin(theta_rad));
                // Add 0.25 floor so axis directions show finite level
                const D_eff=0.25+0.75*D_dipole;
                // dBA at r_ref in this direction
                const spread=20*Math.log10(r_ref);
                const dir_corr=10*Math.log10(Math.max(D_eff,1e-4));
                const dBA_dir=+(dBA_1m-spread+dir_corr).toFixed(1);
                // Convert to polar (x,y) with radius = dBA mapped to 0-1
                const clamp=Math.max(dBA_dir,30);
                return{
                  theta:theta_deg,
                  dBA:dBA_dir,
                  D:+D_eff.toFixed(3),
                  // For recharts radar: need named keys
                  "dBA (dir)":+Math.max(dBA_dir,30).toFixed(1),
                };
              });

              // Key angles
              const dBA_above  = +(dBA_1m - 20*Math.log10(r_ref) + 10*Math.log10(0.25)).toFixed(1);  // θ=0
              const dBA_inplane= +(dBA_1m - 20*Math.log10(r_ref) + 10*Math.log10(1.0)).toFixed(1);   // θ=90
              const dBA_below  = dBA_above; // symmetric

              // 8-point cardinal radar
              const radarData=[0,45,90,135,180,225,270,315].map(deg=>{
                const theta_rad=deg*Math.PI/180;
                const D_eff=0.25+0.75*Math.abs(Math.sin(theta_rad));
                const spread=20*Math.log10(r_ref);
                const dBA_d=+(dBA_1m-spread+10*Math.log10(D_eff)).toFixed(1);
                const labels=["Above (0°)","45°","In-Plane (90°)","135°","Below (180°)","225°","In-Plane (270°)","315°"];
                return{angle:labels[deg/45],dBA:Math.max(dBA_d,25)};
              });

              // Distance propagation for 3 directions
              const distData=[5,10,25,50,100,150,200,300,500].map(r=>{
                const spread=20*Math.log10(r);
                return{
                  r,
                  "In-Plane":+(dBA_1m-spread+10*Math.log10(1.0)).toFixed(1),
                  "45° Down": +(dBA_1m-spread+10*Math.log10(0.25+0.75*Math.abs(Math.sin(Math.PI/4)))).toFixed(1),
                  "Above":    +(dBA_1m-spread+10*Math.log10(0.25)).toFixed(1),
                };
              });

              // SVG polar plot
              const CX=200,CY=200,PR=160;
              const dBAmin=Math.max(dBA_inplane-40,20), dBAmax=dBA_inplane+5;
              const polar_r=(dBA)=>Math.max(0,PR*(dBA-dBAmin)/(dBAmax-dBAmin));

              const polarPath=Array.from({length:N_pts+1},(_,i)=>{
                const idx=i%N_pts;
                const {theta,dBA}=directivityData[idx];
                const r=polar_r(Math.max(dBA,dBAmin));
                const a=(theta-90)*Math.PI/180;
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:"14px 18px"}}>
                  <div style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",letterSpacing:"0.15em",marginBottom:2}}>CERTIFICATION — FAA/EASA UAM</div>
                  <div style={{fontSize:20,fontWeight:800,color:SC.blue}}>Acoustic Directivity Plot</div>
                  <div style={{fontSize:10,color:SC.muted,marginTop:2}}>Dipole (Gutin) directivity model — D(θ)=|sin θ|. θ=90° in-plane is loudest. Reference distance {r_ref}m. Required for FAA Part 36 / EASA CS-36 UAM certification.</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                  <KPI label="In-Plane (max)" value={dBA_inplane} unit="dBA" color={SR.dBA_150m<=65?SC.green:SC.red} sub={`at ${r_ref}m`}/>
                  <KPI label="Above Rotor" value={dBA_above} unit="dBA" color={SC.teal} sub={`at ${r_ref}m, θ=0°`}/>
                  <KPI label="Front/Aft" value={(+(dBA_1m-20*Math.log10(r_ref)+10*Math.log10(0.25+0.75*Math.abs(Math.sin(45*Math.PI/180)))).toFixed(1))} unit="dBA" color={SC.blue} sub={`at ${r_ref}m, θ=45°`}/>
                  <KPI label="Directivity Ratio" value={(dBA_inplane-dBA_above).toFixed(1)} unit="dB" color={SC.amber} sub="in-plane vs above"/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:12,alignItems:"start"}}>
                  {/* SVG Polar Plot */}
                  <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14}}>
                    <div style={{fontSize:10,fontWeight:700,color:SC.text,marginBottom:8,fontFamily:"'DM Mono',monospace"}}>POLAR DIRECTIVITY — {r_ref}m</div>
                    <svg width="400" height="400" viewBox="0 0 400 400">
                      {/* Concentric reference rings */}
                      {[0.25,0.5,0.75,1.0].map(frac=>{
                        const rd=PR*frac;
                        const dBAlbl=(dBAmin+(dBAmax-dBAmin)*frac).toFixed(0);
                        return<g key={frac}>
                          <circle cx={CX} cy={CY} r={rd} fill="none" stroke={SC.border} strokeWidth={0.8}/>
                          <text x={CX+rd+3} y={CY+4} fontSize={8} fill={SC.muted} fontFamily="DM Mono,monospace">{dBAlbl} dBA</text>
                        </g>;
                      })}
                      {/* Axis spokes (every 30°) */}
                      {Array.from({length:12},(_,i)=>{
                        const a=(i*30-90)*Math.PI/180;
                        const label_deg=i*30;
                        return<g key={i}>
                          <line x1={CX} y1={CY} x2={CX+PR*1.1*Math.cos(a)} y2={CY+PR*1.1*Math.sin(a)}
                            stroke={SC.border} strokeWidth={0.5}/>
                          <text x={CX+PR*1.18*Math.cos(a)-8} y={CY+PR*1.18*Math.sin(a)+4}
                            fontSize={9} fill={SC.muted} fontFamily="DM Mono,monospace" textAnchor="middle">{label_deg}°</text>
                        </g>;
                      })}
                      {/* EASA 65 dBA limit ring */}
                      {(()=>{
                        const r65=polar_r(65);
                        return r65>0?<circle cx={CX} cy={CY} r={r65} fill="none" stroke={SC.green}
                          strokeWidth={1.5} strokeDasharray="5 3" opacity={0.7}/>:null;
                      })()}
                      {/* Directivity lobe */}
                      <path d={polarPath} fill={SC.blue+"33"} stroke={SC.blue} strokeWidth={2}/>
                      {/* Aircraft icon */}
                      <text x={CX-6} y={CY+6} fontSize={16} style={{userSelect:"none"}}>✈️</text>
                      {/* Labels */}
                      <text x={CX} y={CY-PR*1.35} textAnchor="middle" fontSize={9} fill={SC.green} fontFamily="DM Mono,monospace">ABOVE (quiet)</text>
                      <text x={CX+PR*1.3} y={CY+4} textAnchor="start" fontSize={9} fill={SC.red} fontFamily="DM Mono,monospace">IN-PLANE (loud)</text>
                      <text x={CX} y={CY+PR*1.45} textAnchor="middle" fontSize={9} fill={SC.muted} fontFamily="DM Mono,monospace">BELOW</text>
                    </svg>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    {/* Radar chart */}
                    <Panel title="8-Direction dBA Level">
                      <ResponsiveContainer width="100%" height={240}>
                        <RadarChart data={radarData} margin={{top:10,right:30,left:30,bottom:10}}>
                          <PolarGrid stroke={SC.border}/>
                          <PolarAngleAxis dataKey="angle" tick={{fontSize:9,fill:SC.muted}}/>
                          <Radar dataKey="dBA" stroke={SC.blue} fill={SC.blue} fillOpacity={0.25} name="dBA"/>
                          <Tooltip {...TTP} formatter={v=>[`${v} dBA`,"SPL"]}/>
                        </RadarChart>
                      </ResponsiveContainer>
                    </Panel>
                    {/* Propagation curves by direction */}
                    <Panel title="SPL vs Distance — Three Observer Directions">
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={distData} margin={{top:5,right:20,left:5,bottom:20}}>
                          <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                          <XAxis dataKey="r" tick={{fontSize:10,fill:SC.muted}}
                            label={{value:"Distance (m)",position:"insideBottom",offset:-10,fontSize:11,fill:SC.muted}}/>
                          <YAxis domain={[30,Math.ceil(dBA_1m/10)*10]} tick={{fontSize:10,fill:SC.muted}}
                            label={{value:"dBA",angle:-90,position:"insideLeft",fontSize:11,fill:SC.muted}}/>
                          <Tooltip {...TTP}/>
                          <ReferenceLine y={65} stroke={SC.green} strokeDasharray="4 3"
                            label={{value:"65 dBA UAM",fill:SC.green,fontSize:9,position:"right"}}/>
                          <ReferenceLine y={75} stroke={SC.amber} strokeDasharray="4 3"
                            label={{value:"75 dBA FAA",fill:SC.amber,fontSize:9,position:"right"}}/>
                          <Line type="monotone" dataKey="In-Plane" stroke={SC.red} strokeWidth={2} dot={false}/>
                          <Line type="monotone" dataKey="45° Down" stroke={SC.amber} strokeWidth={2} dot={false}/>
                          <Line type="monotone" dataKey="Above" stroke={SC.teal} strokeWidth={2} dot={false}/>
                          <Legend iconSize={10} wrapperStyle={{fontSize:10,color:SC.muted}}/>
                        </LineChart>
                      </ResponsiveContainer>
                    </Panel>
                  </div>
            ...
            {/* ──── TAB 26: STRUCTURAL LOAD PATH VISUALIZER ──── */}
            {tab===26&&SR&&(()=>{
              const Vs=SR.Vstall, Va=SR.VA, Vc=params.vCruise, Vd=SR.VD;
              const nPos=3.5, nNeg=-1.5, nGust=2.0;

              // For each structural member, determine governing load case from V-n data
              // Load cases: Maneuver+, Maneuver-, Gust Up, Gust Down, Dive
              const loadCases=[
                {id:"MAN+", label:"Maneuver +n", n:nPos, speed:Va, color:"#ef4444", icon:"⬆"},
                {id:"MAN-", label:"Maneuver -n", n:nNeg, speed:Va, color:"#3b82f6", icon:"⬇"},
                {id:"GUST+",label:"Gust Up",    n:nGust,speed:Vc, color:"#f59e0b", icon:"💨"},
                {id:"GUST-",label:"Gust Down",  n:-nGust,speed:Vc,color:"#8b5cf6",icon:"💨"},
                {id:"DIVE", label:"Dive",        n:1.0,  speed:Vd, color:"#06d6a0", icon:"🔽"},
              ];

              // Member definitions with governing case logic
              const MTOW=SR.MTOW, g0=9.81;
              const W=MTOW*g0;
              const members=[
                {
                  id:"wing_root",label:"Wing Root Bending",section:"Wing",
                  desc:"Primary wing-to-fuselage spar carry-through. Governs airframe structural weight.",
                  load_man_pos:+(nPos*W*SR.bWing/8/1000).toFixed(1),
                  load_man_neg:+(Math.abs(nNeg)*W*SR.bWing/8/1000).toFixed(1),
                  load_gust:+(nGust*W*SR.bWing/8/1000).toFixed(1),
                  load_dive:+(1.0*W*SR.bWing/8/1000).toFixed(1),
                  unit:"kN·m",governing:"MAN+",govFactor:nPos,
                },
                {
                  id:"wing_shear",label:"Wing Root Shear",section:"Wing",
                  desc:"Vertical shear force at wing root. Must be transferred through shear web.",
                  load_man_pos:+(nPos*W*0.5/1000).toFixed(1),
                  load_man_neg:+(Math.abs(nNeg)*W*0.5/1000).toFixed(1),
                  load_gust:+(nGust*W*0.5/1000).toFixed(1),
                  load_dive:+(W*0.5/1000).toFixed(1),
                  unit:"kN",governing:"MAN+",govFactor:nPos,
                },
                {
                  id:"fuselage_keel",label:"Fuselage Keel Beam",section:"Fuselage",
                  desc:"Primary longitudinal load path. Compression from bending, tension from landing.",
                  load_man_pos:+(nPos*W*params.fusLen/4/1000).toFixed(1),
                  load_man_neg:+(Math.abs(nNeg)*W*params.fusLen/4/1000).toFixed(1),
                  load_gust:+(nGust*W*params.fusLen/4/1000).toFixed(1),
                  load_dive:+(W*params.fusLen/4/1000).toFixed(1),
                  unit:"kN",governing:"MAN+",govFactor:nPos,
                },
                {
                  id:"vtail_attach",label:"V-Tail Attachment",section:"Empennage",
                  desc:"Ruddervator hinge & carry-through. Gust loads often govern tail structures.",
                  load_man_pos:+(0.3*W*SR.lv/1000).toFixed(1),
                  load_man_neg:+(0.2*W*SR.lv/1000).toFixed(1),
                  load_gust:+(0.35*W*SR.lv/1000).toFixed(1),
                  load_dive:+(0.25*W*SR.lv/1000).toFixed(1),
                  unit:"kN·m",governing:"GUST+",govFactor:nGust,
                },
                {
                  id:"motor_mount",label:"Motor / Rotor Mount",section:"Propulsion",
                  desc:"Rotor hub loads transmitted to wing/boom structure. Fatigue critical at BPF.",
                  load_man_pos:+(nPos*SR.Trotor/1000).toFixed(1),
                  load_man_neg:+(Math.abs(nNeg)*SR.Trotor/1000).toFixed(1),
                  load_gust:+(nGust*SR.Trotor/1000).toFixed(1),
                  load_dive:+(SR.Trotor/1000).toFixed(1),
                  unit:"kN",governing:"MAN+",govFactor:nPos,
                },
                {
                  id:"landing_gear",label:"Landing Gear Strut",section:"Landing Gear",
                  desc:"Absorbs touchdown vertical velocity. FAR 27.725: ultimate = limit × 1.5.",
                  load_man_pos:+(2.5*W/1000).toFixed(1),
                  load_man_neg:+(0.5*W/1000).toFixed(1),
                  load_gust:+(1.5*W/1000).toFixed(1),
                  load_dive:+(0.8*W/1000).toFixed(1),
                  unit:"kN",governing:"DIVE",govFactor:2.5,
                },
                {
                  id:"battery_frame",label:"Battery Frame Rails",section:"Systems",
                  desc:"Carry battery mass (crashworthiness & maneuver). EUROCAE / DO-311A floor load.",
                  load_man_pos:+(nPos*SR.Wbat*g0/1000).toFixed(1),
                  load_man_neg:+(Math.abs(nNeg)*SR.Wbat*g0/1000).toFixed(1),
                  load_gust:+(nGust*SR.Wbat*g0/1000).toFixed(1),
                  load_dive:+(SR.Wbat*g0/1000).toFixed(1),
                  unit:"kN",governing:"MAN+",govFactor:nPos,
                },
              ];

              const lcColor={
                "MAN+":"#ef4444","MAN-":"#3b82f6",
                "GUST+":"#f59e0b","GUST-":"#8b5cf6","DIVE":"#06d6a0"
              };
              const lcLabel={"MAN+":"Maneuver +n","MAN-":"Maneuver -n","GUST+":"Gust Up","GUST-":"Gust Down","DIVE":"Dive"};

              // Build bar chart data
              const barData=members.map(m=>({
                name:m.label.replace(" Root","").replace(" Keel Beam","").replace(" / Rotor","").replace(" Frame Rails",""),
                "Maneuver +n":+m.load_man_pos,
                "Gust Up":+m.load_gust,
                "Maneuver -n":+m.load_man_neg,
                Dive:+m.load_dive,
              }));

              // V-n load factor bar
              const vnBarData=SR.vnData.filter((_,i)=>i%5===0).map(d=>({
                v:d.v,nPos:+d.nPos.toFixed(2),nNeg:+d.nNeg.toFixed(2)
              }));

              return(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:"14px 18px"}}>
                  <div style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",letterSpacing:"0.15em",marginBottom:2}}>STRUCTURAL ANALYSIS</div>
                  <div style={{fontSize:20,fontWeight:800,color:SC.red}}>Structural Load Path Visualizer</div>
                  <div style={{fontSize:10,color:SC.muted,marginTop:2}}>Governing load case per structural member derived from V-n diagram. Color = governing flight condition. Ultimate = Limit × 1.5 (FAR 27 / CS-27).</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
                  {loadCases.map(lc=>(
                    <div key={lc.id} style={{background:SC.panel,border:`1px solid ${lc.color}44`,
                      borderLeft:`3px solid ${lc.color}`,borderRadius:6,padding:"8px 10px"}}>
                      <div style={{fontSize:10,fontWeight:700,color:lc.color,fontFamily:"'DM Mono',monospace"}}>{lc.icon} {lc.label}</div>
                      <div style={{fontSize:9,color:SC.muted,marginTop:2}}>n = {lc.n > 0 ? "+" : ""}{lc.n.toFixed(1)} @ {lc.speed.toFixed(1)} m/s</div>
                    </div>
                  ))}
                </div>
                {/* Member table */}
                <Panel title="Structural Members — Governing Load Case & Limit Loads">
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"'DM Mono',monospace"}}>
                      <thead><tr style={{borderBottom:`1px solid ${SC.border}`}}>
                        {["Member","Section","Governing Case","Limit Load","Ult. Load (×1.5)","Load Cases","Description"].map(h=>(
                          <th key={h} style={{padding:"4px 8px",textAlign:"left",fontSize:8,color:SC.muted,
                            textTransform:"uppercase",letterSpacing:"0.06em",whiteSpace:"nowrap"}}>{h}</th>
                        ))}</tr></thead>
                      <tbody>{members.map((m,i)=>{
                        const govCol=lcColor[m.governing]||SC.amber;
                        const limitLoad=Math.max(m.load_man_pos,m.load_man_neg,m.load_gust,m.load_dive);
                        const ultLoad=+(limitLoad*1.5).toFixed(1);
                        return<tr key={m.id} style={{background:i%2===0?SC.bg:"transparent",
                          borderBottom:`1px solid ${SC.border}22`}}>
                          <td style={{padding:"5px 8px",color:SC.amber,fontWeight:700,whiteSpace:"nowrap"}}>{m.label}</td>
                          <td style={{padding:"5px 8px",color:SC.muted,fontSize:9}}>{m.section}</td>
                          <td style={{padding:"5px 8px"}}>
                            <span style={{background:govCol+"22",color:govCol,padding:"2px 6px",
                              borderRadius:3,fontSize:9,fontWeight:700,border:`1px solid ${govCol}44`}}>
                              {lcLabel[m.governing]}
                            </span>
                          </td>
                          <td style={{padding:"5px 8px",color:govCol,fontWeight:700}}>{limitLoad.toFixed(1)} {m.unit}</td>
                          <td style={{padding:"5px 8px",color:SC.red,fontWeight:700}}>{ultLoad} {m.unit}</td>
                          <td style={{padding:"5px 8px"}}>
                            <div style={{display:"flex",gap:3}}>
                              {[["MAN+",m.load_man_pos],["GUST+",m.load_gust],["MAN-",m.load_man_neg],["DIVE",m.load_dive]].map(([id,val])=>{
                                const isGov=id===m.governing;
                                return<div key={id} style={{fontSize:7,padding:"1px 4px",borderRadius:2,
                                  background:isGov?lcColor[id]+"33":"transparent",
                                  color:lcColor[id],border:`1px solid ${lcColor[id]}44`,
                                  fontWeight:isGov?800:400,whiteSpace:"nowrap"}}>
                                  {id}: {val}
                                </div>;
                              })}
                            </div>
                          </td>
                          <td style={{padding:"5px 8px",color:SC.muted,fontSize:9,maxWidth:200}}>{m.desc}</td>
                        </tr>;
                      })}</tbody>
                    </table>
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  <Panel title="Limit Load by Member & Load Case">
                    <ResponsiveContainer width="100%" height={260}>
                      <BarChart data={barData} margin={{top:5,right:10,left:10,bottom:60}}>
                        <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                        <XAxis dataKey="name" tick={{fontSize:8,fill:SC.muted}} angle={-35} textAnchor="end" interval={0}/>
                        <YAxis tick={{fontSize:9,fill:SC.muted}} label={{value:"Load",angle:-90,position:"insideLeft",fontSize:10,fill:SC.muted}}/>
                        <Tooltip {...TTP}/>
                        <Legend iconSize={8} wrapperStyle={{fontSize:9,color:SC.muted,paddingTop:4}}/>
                        <Bar dataKey="Maneuver +n" fill="#ef4444" opacity={0.8} stackId={undefined}/>
                        <Bar dataKey="Gust Up"     fill="#f59e0b" opacity={0.8}/>
                        <Bar dataKey="Maneuver -n" fill="#3b82f6" opacity={0.8}/>
                        <Bar dataKey="Dive"        fill="#06d6a0" opacity={0.8}/>
                      </BarChart>
                    </ResponsiveContainer>
                  </Panel>
                  <Panel title="V-n Envelope — Load Factor vs Airspeed">
                    <ResponsiveContainer width="100%" height={260}>
                      <ComposedChart data={vnBarData} margin={{top:5,right:20,left:5,bottom:20}}>
                        <CartesianGrid strokeDasharray="2 2" stroke={SC.border}/>
                        <XAxis dataKey="v" tick={{fontSize:9,fill:SC.muted}}
                          label={{value:"Airspeed (m/s)",position:"insideBottom",offset:-10,fontSize:11,fill:SC.muted}}/>
                        <YAxis domain={[-2.5,4.5]} tick={{fontSize:9,fill:SC.muted}}
                          label={{value:"Load factor n",angle:-90,position:"insideLeft",fontSize:11,fill:SC.muted}}/>
                        <Tooltip {...TTP}/>
                        <ReferenceLine y={0} stroke={SC.muted} strokeWidth={0.5}/>
                        <ReferenceLine y={3.5} stroke="#ef4444" strokeDasharray="4 3"
                          label={{value:"n=+3.5 limit",fill:"#ef4444",fontSize:9}}/>
                        <ReferenceLine y={-1.5} stroke="#3b82f6" strokeDasharray="4 3"
                          label={{value:"n=−1.5 limit",fill:"#3b82f6",fontSize:9}}/>
                        <ReferenceLine x={Va} stroke={SC.green} strokeDasharray="4 3"
                          label={{value:"Va",fill:SC.green,fontSize:9,position:"top"}}/>
                        <ReferenceLine x={Vc} stroke={SC.amber} strokeDasharray="4 3"
                          label={{value:"Vc",fill:SC.amber,fontSize:9,position:"top"}}/>
                        <ReferenceLine x={Vd} stroke={SC.red} strokeDasharray="4 3"
                          label={{value:"Vd",fill:SC.red,fontSize:9,position:"top"}}/>
                        {/* Shade the flight envelope */}
                        <Area type="monotone" dataKey="nPos" stroke="#ef4444" fill="#ef444411"
                          strokeWidth={2} dot={false} name="+n envelope"/>
                        <Area type="monotone" dataKey="nNeg" stroke="#3b82f6" fill="#3b82f611"
                          strokeWidth={2} dot={false} name="−n envelope"/>
                      </ComposedChart>
                    </ResponsiveContainer>
                  </Panel>
                </div>
                {/* Structural margin summary */}
                <Panel title="Structural Design Summary — FAR 27 / CS-27 Requirements">
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                    {[
                      ["Design Limit Load Factor",`+${nPos} / ${nNeg}`,SC.text,"FAR 27.337"],
                      ["Gust Load Factor",`±${nGust}`,SC.text,"FAR 27.341"],
                      ["Ultimate Load Factor",`+${(nPos*1.5).toFixed(1)} / ${(nNeg*1.5).toFixed(1)}`,SC.red,"FAR 27.303 (×1.5)"],
                      ["Corner Speed Va",`${Va.toFixed(1)} m/s`,SC.green,"Maneuver onset"],
                      ["Cruise Speed Vc",`${Vc.toFixed(1)} m/s`,SC.amber,"Design cruise"],
                      ["Dive Speed Vd",`${Vd.toFixed(1)} m/s`,SC.red,"1.25 × Vc"],
                      ["MTOW",`${SR.MTOW} kg`,SC.text,"Sizing basis"],
                      ["Wing Loading",`${SR.WL.toFixed(1)} N/m²`,SC.text,"Aerodynamic ref"],
                      ["Governing Member","Wing Root Bending",SC.amber,"Highest limit load"],
                    ].map(([lbl,val,col,ref])=>(
                      <div key={lbl} style={{background:SC.bg,borderRadius:4,padding:"8px 10px",
                        border:`1px solid ${SC.border}`}}>
                        <div style={{fontSize:8,color:SC.muted,fontFamily:"'DM Mono',monospace",marginBottom:2,textTransform:"uppercase"}}>{lbl}</div>
                        <div style={{fontSize:13,fontWeight:700,color:col,fontFamily:"'DM Mono',monospace"}}>{val}</div>
                        <div style={{fontSize:8,color:SC.dim,marginTop:2}}>{ref}</div>
                      </div>
                    ))}
                  </div>
                </Panel>
