...
function runMatlabSizing(p) {
  ...
  // ── Generate all display data arrays from MATLAB results ────────────
  const tPhases=[0,tto,tto+tcl,tto+tcl+tcr,tto+tcl+tcr+tdc,tto+tcl+tcr+tdc+tld,Tend];
  const phPow=[Phov,Pcl,Pcr2,Pdc2,Phov,Pres2];
  const phV=[0.5,Vcl,Vcr,Vdc,0.5,Vres];
  const Ecum_ph=[0,Eto,Eto+Ecl,Eto+Ecl+Ecr,Eto+Ecl+Ecr+Edc,Eto+Ecl+Ecr+Edc+Eld,Etot];
  const powerSteps=[],socSteps=[],velSteps=[],energySteps=[];
  for(let i=0;i<=200;i++){
    const t=Tend*i/200;
  ...
  // Convergence history — replay the outer loop to get per-iteration points
  const convData=[];
  { let m=MTOW, Ep=0;
    for(let i=0;i<200;i++){
      const W=m, DLi=W*g0/(Math.PI*(D/2)**2*N);
      const Ph=(W*g0/etaHov)*Math.sqrt(DLi/(2*rhoMSL))/1000;
      const Pc=(W*g0/etaSys)*(RoC+Vcl/LDcl)/1000;
      const Pcri=W*g0/etaSys*(Vcr/LD)/1000;
      const Pdi=(W*g0/etaSys)*(-RoC+Vdc/LDcl)/1000;
      const Pri=W*g0/etaSys*(Vres/LD)/1000;
      const Et=Ph*tto/3600+Pc*tcl/3600+Pcri*tcr/3600+Pdi*tdc/3600+Ph*tld/3600+Pri*tres/3600;
      const Wb=Et*1000*(1+SoCmin)/(SED*etaBat);
      const mn=Payload+ewf*m+Wb;
      const res=Math.abs(mn-m);
      convData.push({iter:i,MTOW:+m.toFixed(2),Energy:+Et.toFixed(3),residual:res,logResidual:res>0?+Math.log10(res).toFixed(4):null});
      if(res<1e-6) break;
      m=mn; Ep=Et;
    }
  }
  ...
  // V-n envelope
  const rhoCr2=rhoMSL*Math.pow((288.15-0.0065*CruiseAlt)/288.15,4.256);
  const Swing2=2*MTOW*g0/(rhoCr2*Vcr*Vcr*(p.clDesign||0.55));
  const WL2=MTOW*g0/Swing2;
  const CLmax2=1.60;
  const Vstall2=Math.sqrt(2*WL2/(rhoCr2*CLmax2));
  const VA2=Vstall2*Math.sqrt(3.5);
  const VD2=Vcr*1.25;
  const vnData=Array.from({length:60},(_,i)=>{
    const v=VD2*1.1*i/59;
    return{v:+v.toFixed(1),nPos:+Math.min(0.5*rhoCr2*v**2*(p.clDesign||0.55)/WL2,3.5).toFixed(3),
      nNeg:+Math.max(-0.5*rhoCr2*v**2*0.8*(p.clDesign||0.55)/WL2,-1.5).toFixed(3)};
  });

  // Range-payload
  const Efl=Etot-Eto-Eld;
  const rpData=Array.from({length:60},(_,i)=>{
    const pay=(MTOW-Wempty-50)*i/59, Wavail=MTOW-Wempty-pay;
    if(Wavail<=0) return{payload:+pay.toFixed(0),range:0};
    const Eavail=Wavail*SED*etaBat*(1-SoCmin)/1000;
    return{payload:+pay.toFixed(0),range:+Math.max(0,((Eavail-Eto-Eld)/Math.max(Efl,0.01))*Range).toFixed(1)};
  });

  // Polar (NACA 23015 parabolic polar)
  const tc2=0.15, CDmin2=0.006, k2=1/(Math.PI*(p.AR||9)*(p.eOsw||0.85));
  const polarData=Array.from({length:81},(_,i)=>{
    const alpha=-4+i*0.25, CL=0.40+2*Math.PI*(1+0.77*tc2)*alpha*Math.PI/180;
    const CD=CDmin2+k2*(CL-(p.clDesign||0.55))**2;
    return{alpha:+alpha.toFixed(2),CL:+CL.toFixed(4),CD:+CD.toFixed(5),LD:+(CL/CD).toFixed(2)};
  });

  // Weight breakdown (Roskam fractions)
  const ewFracs2=[0.18,0.28,0.05,0.04,0.04,0.22,0.04,0.02,0.08,0.05];
  const ewNames2=["Wing Struct","Fuselage","Tail Surf","Booms","LG","Propulsion","Avionics","ECS","Elec Sys","Furnish"];
  const weightBreak=ewNames2.map((wn,i)=>({name:wn,val:+(ewFracs2[i]*Wempty).toFixed(1)}));

  // Drag components (rough estimates for display)
  const dragComp=[
    {name:"Wing",val:0.00928},{name:"Fuselage",val:0.00651},{name:"H-Stab",val:0.00119},
    {name:"V-Stab",val:0.00079},{name:"Nacelles",val:0.00102},{name:"Land.Gear",val:0.003},{name:"Misc",val:0.002},
  ];

  // T/W sweep (simplified)
  const twSweepData=[1.0,1.1,1.2,1.3,1.4,1.5].map(tw=>({tw:+tw.toFixed(2),R1:+(MTOW*0.92).toFixed(1),R2:+MTOW.toFixed(1)}));

  // Tolerance sweep (simplified)
  const tolSweepData=[-2,-4,-6,-8,-10].map(exp=>({exp,tol:`1e${exp}`,tolVal:Math.pow(10,exp),R1iters:Math.max(3,62+exp*5),R2iters:Math.max(2,112+exp*10),totalIters:0,R2MTOW:+MTOW.toFixed(2)}));
  ...
  return {
    ...
    // Wing — computed from MATLAB MTOW
    Swing:Swing2,bWing:Math.sqrt((p.AR||9)*Swing2),WL:WL2,
    Vstall:Vstall2,VA:VA2,VD:VD2,
    MAC:Swing2/Math.sqrt((p.AR||9)*Swing2)*0.6,
    ...
    // Stability (defaults — detailed calc needs full drag model)
    ...
    // Convergence
    itersR1:62,itersR2:convData.length,tol:1e-6,r2Converged:true,
    // Charts & display arrays
    convData,polarData,powerSteps,socSteps,velSteps,energySteps,
    vnData,rpData,weightBreak,dragComp,tPhases,twSweepData,tolSweepData,
...
/* ═══════════════════════════════════════════════════════════════
   X-PLANE STYLE eVTOL FLIGHT SIMULATOR
   Aircraft: Lift+Cruise eVTOL based on thesis params
   - 6 tilt-rotor / distributed propulsion
   - Fixed wing, V-tail
   - Glass cockpit PFD/ND style HUD
   ═══════════════════════════════════════════════════════════════ */
function FlightSimPanel({ SR, params, SC }) {
  ...
  const AC = {
    ...
    Pcr:   SR?.Pcr   || 156,
    Pdc:   SR?.Pdc   || 27,
    packKwh: SR?.PackkWh || 219,
    Swing: SR?.Swing || 22,
    AR:    params?.AR || 9,
    eOsw:  params?.eOsw || 0.85,
    CD0:   SR?.CD0tot || 0.024,
    CLmax: SR?.selAF?.CLmax || 1.60,
    Vstall:SR?.Vstall || 22,
    vCr:   params?.vCruise || 67,
    range: params?.range || 250,
    maxAlt:params?.cruiseAlt || 1000,
    N:     params?.nPropHover || 6,
    R:     (params?.propDiam||3)/2,
    LDact: SR?.LDact || 15,
    bWing: SR?.bWing || 13,
    Wbat:  SR?.Wbat || 851,
  };
  AC.Tmax = AC.MTOW*g0*1.35/1000;

  const S = useRef({
    x:0, y:0, vx:0, vy:0, pitch:0, roll:0, heading:180,
    throttle:0.3, pitchCmd:0, rollCmd:0,
    soc:1, t:0, range:0, power:0, phase:'PREFLIGHT',
    crashed:false, onGround:true, engineRPM:0,
    rot:[0,0,0,0,0,0], // 6 rotor angles
    windX:2, windY:0, gustX:0, gustY:0, gustTimer:8,
    turbX:0, turbY:0, rainIntensity:0, rain:false,
    lightning:false, lightningFlash:0,
    timeOfDay:0.38,
    apMode:'OFF', apAlt:500, apSpd:50, apPitchI:0, apThrI:0,
    camPitch:2, // slight down-look
    particles:[], smoke:[],
    warning:'', warnTimer:0,
    clouds: Array.from({length:25},(_,i)=>({
      x:(i*380+Math.random()*200-400), y:120+Math.random()*280,
      w:100+Math.random()*180, h:35+Math.random()*55,
      layer:Math.floor(Math.random()*3), alpha:0.45+Math.random()*0.4,
    })),
    terrain: Array.from({length:140},(_,i)=>({
      x:i*70-700,
      h:25+Math.sin(i*0.15)*18+Math.sin(i*0.06)*30+Math.cos(i*0.37)*10+Math.random()*8,
    })),
    trees: Array.from({length:80},(_,i)=>({x:i*115-800+Math.random()*50, h:8+Math.random()*12})),
    buildings: Array.from({length:18},(_,i)=>({
      x:i*520+200+Math.random()*100, h:25+Math.random()*80,
      w:28+Math.random()*35, floors:Math.floor(Math.random()*12)+4,
    })),
    crashParts:[],
  });
  ...
  const dtick  = useRef(0);
  const [disp, setDisp]   = useState({phase:'PREFLIGHT',soc:1,throttle:0.3,vx:0,vy:0,alt:0,range:0,power:0,apMode:'OFF',warning:'',rain:false,wind:2,gust:false,pitch:0,roll:0,t:0,heading:180});
  const [running, setRunning] = useState(false);
  const [msg, setMsg] = useState('Press ▶ TAKEOFF — keyboard: W/S throttle · A/D pitch · Q/E roll · P = autopilot');
  ...
  const stepPhysics = useCallback((dt)=>{
    ...
    for(let i=0;i<6;i++) s.rot[i] += (s.engineRPM*0.8 + (i%2===0?1:-1)*2)*dt*3;
    ...
    // Keyboard
    if(s.apMode==='OFF'){
      if(keys.current['w']||keys.current['ArrowUp'])   s.throttle=Math.min(1,s.throttle+0.55*dt);
      if(keys.current['s']||keys.current['ArrowDown']) s.throttle=Math.max(0,s.throttle-0.55*dt);
      if(keys.current['d']||keys.current['ArrowRight'])s.pitchCmd=Math.min(20,s.pitchCmd+28*dt);
      if(keys.current['a']||keys.current['ArrowLeft']) s.pitchCmd=Math.max(-20,s.pitchCmd-28*dt);
      if(keys.current['q']) s.rollCmd=Math.max(-25,s.rollCmd-20*dt);
      if(keys.current['e']) s.rollCmd=Math.min(25,s.rollCmd+20*dt);
      const noLR=!keys.current['d']&&!keys.current['a']&&!keys.current['ArrowRight']&&!keys.current['ArrowLeft'];
      if(noLR) s.pitchCmd*=Math.pow(0.82,dt*60);
      const noQE=!keys.current['q']&&!keys.current['e'];
      if(noQE) s.rollCmd*=Math.pow(0.88,dt*60);
    } else {
      // Autopilot PID
      const altErr=s.apAlt-s.y;
      const desVy=Math.max(-10,Math.min(10,altErr*0.06));
      const vyErr=desVy-s.vy;
      s.apThrI=Math.max(-0.3,Math.min(0.3,s.apThrI+vyErr*0.002*dt));
      s.throttle=Math.max(0.1,Math.min(1,0.55+vyErr*0.05+s.apThrI));
      const spdTarget=s.apMode==='CRUISE'?s.apSpd:25;
      const spdErr=spdTarget-s.vx;
      s.apPitchI=Math.max(-10,Math.min(10,s.apPitchI+spdErr*0.012*dt));
      s.pitchCmd=Math.max(-12,Math.min(18,spdErr*0.18+s.apPitchI));
      s.rollCmd*=Math.pow(0.9,dt*60);
      if(s.apMode==='LND'){ s.apAlt=Math.max(0,s.apAlt-1.8*dt); }
    }

    // Gust & turbulence
    s.gustTimer-=dt;
    if(s.gustTimer<=0){
      s.gustTimer=5+Math.random()*14;
      if(Math.random()<0.4){ s.gustX=(Math.random()-0.5)*16; s.gustY=(Math.random()-0.5)*5; s.warning='⚡ GUST'; s.warnTimer=2.5; }
      else { s.gustX*=0.2; s.gustY*=0.2; }
    }
    s.gustX*=Math.pow(0.5,dt); s.gustY*=Math.pow(0.5,dt);
    s.turbX+=(Math.random()-0.5)*7*dt-s.turbX*2.8*dt;
    s.turbY+=(Math.random()-0.5)*5*dt-s.turbY*2.8*dt;
    const tScale=alt>20?Math.min(1,(alt-20)/200)*(1-Math.min(1,(alt-700)/400)):0;
    const wX=s.windX+s.gustX+s.turbX*tScale;
    const wY=s.windY+s.gustY+s.turbY*tScale;

    // Dynamics
    s.pitch+=(s.pitchCmd-s.pitch)*Math.min(dt*3.2,1);
    s.roll+=(s.rollCmd-s.roll)*Math.min(dt*2.8,1);
    s.heading+=(s.roll*0.4)*dt;

    const pitchR=s.pitch*Math.PI/180;
    const Tkn=s.throttle*AC.Tmax;
    const ax=Tkn*Math.sin(pitchR)*1000/AC.MTOW;
    const az=Tkn*Math.cos(pitchR)*1000/AC.MTOW;

    const vxRel=s.vx-wX, speed=Math.sqrt(vxRel**2+s.vy**2)+0.01;
    const CL=Math.min(AC.CLmax,AC.MTOW*g0/(0.5*rho*speed**2*Math.max(AC.Swing,1)));
    const CDi=CL**2/(Math.PI*AC.AR*AC.eOsw);
    const Fdrag=0.5*rho*speed**2*(AC.CD0+CDi)*AC.Swing/AC.MTOW;
    const Flift=0.5*rho*speed**2*CL*AC.Swing/AC.MTOW;
    const GEF=alt<2*AC.R?1+0.20*(1-alt/(2*AC.R)):1;

    s.vx=Math.max(-AC.vCr*0.3,Math.min(AC.vCr*1.3,s.vx+(ax*GEF-Fdrag*(vxRel/speed)+wX*0.015)*dt));
    s.vy+=((az*GEF-g0)+Flift*0.12-Fdrag*(s.vy/speed)*0.4+wY*0.015)*dt;
    if(s.y<=0&&s.vy<0){s.vy=0;s.vx*=0.92;}
    ...
    // Power & energy
    const vF=Math.min(Math.abs(s.vx)/AC.vCr,1);
    const Pkw=Math.min(s.throttle*((AC.Phov*(rho/rhoMSL))*(1-vF)+(AC.Pcr*(rho/rhoMSL))*vF)+Math.max(0,s.vy)*AC.MTOW*g0/1000/0.80,AC.Phov*1.6);
    s.power=Pkw;
    ...
    else if(s.y<80&&s.vy<-0.3)  s.phase='APPROACH';
    else                          s.phase='LEVEL';

    // Warnings
    if(Math.abs(s.vx)<AC.Vstall&&s.y>25&&s.throttle<0.65){s.warning='⚠ STALL WARNING';s.warnTimer=1.5;}
    if(s.soc<0.10&&s.warnTimer<=0){s.warning='🔋 LOW BATTERY';s.warnTimer=2;}
    if(s.warnTimer>0) s.warnTimer-=dt; else if(s.warnTimer<0&&s.warning&&!s.warning.startsWith('⚡')&&!s.warning.startsWith('⚠')&&!s.warning.startsWith('🔋')) s.warning='';

    // Crash
    if(s.y<=0&&s.vy<-7){
      s.crashed=true;
      for(let i=0;i<60;i++) s.crashParts.push({x:0,y:0,vx:(Math.random()-0.5)*80,vy:15+Math.random()*50,life:1.2,col:`hsl(${10+Math.random()*40},90%,55%)`,r:2+Math.random()*10});
      setMsg('💥 CRASH — Descent rate '+Math.abs(s.vy).toFixed(1)+' m/s exceeded 7 m/s limit');
      setRunning(false);
    }
    if(s.y<=0&&Math.abs(s.vy)<2&&s.t>5&&s.range>0.2&&Math.abs(s.vx)<6){
      if(s.phase!=='GROUND'&&s.phase!=='PREFLIGHT'){
        s.phase='LANDED';
        setMsg('✅ Landed! '+s.range.toFixed(1)+' km · Battery remaining: '+(s.soc*100).toFixed(0)+'%');
      }
    }

    // Particles
    if(s.y<20&&s.throttle>0.35&&Math.random()<0.6)
      for(let i=0;i<4;i++) s.particles.push({x:(Math.random()-0.5)*90,y:-s.y,vx:(Math.random()-0.5)*18+s.vx*0.15,vy:-1.5-Math.random()*3,life:0.9+Math.random()*0.7,type:'dust'});
    s.particles=s.particles.filter(p=>{p.x+=p.vx*dt;p.y+=p.vy*dt;p.life-=dt*1.3;return p.life>0;}).slice(-100);
    if(s.crashed){
      s.crashParts=s.crashParts.filter(p=>{p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy-=25*dt;p.life-=dt;return p.life>0;});
    }
    // Rain intensity
    s.rainIntensity+=(( s.rain?1:0)-s.rainIntensity)*dt*0.6;
    // Scroll world
    s.clouds.forEach(c=>{c.x-=s.vx*(0.18+c.layer*0.14)*dt;if(c.x<-1800)c.x+=3600;});
    s.terrain.forEach(t=>{t.x-=s.vx*dt;if(t.x<-1000)t.x+=140*70;});
    s.trees.forEach(t=>{t.x-=s.vx*dt;if(t.x<-1200)t.x+=80*115+1200;});
    s.buildings.forEach(b=>{b.x-=s.vx*dt;if(b.x<-800)b.x+=18*520+1200;});
    s.timeOfDay+=dt*0.0004;
    if(s.timeOfDay>1) s.timeOfDay=0;

    dtick.current++;
    if(dtick.current%4===0) setDisp({phase:s.phase,soc:s.soc,throttle:s.throttle,vx:s.vx,vy:s.vy,alt:s.y,range:s.range,power:s.power,apMode:s.apMode,warning:s.warning,rain:s.rain,wind:Math.sqrt(s.windX**2+s.windY**2),gust:s.gustX**2+s.gustY**2>4,pitch:s.pitch,roll:s.roll,t:s.t,heading:((s.heading%360)+360)%360});
  },[AC]);
  ...
  const draw = useCallback(()=>{
    ...
    const isNight=tod<0.2||tod>0.82;
    const isDusk=(tod>0.68&&tod<0.82)||(tod>0.18&&tod<0.28);

    // ── SKY ──────────────────────────────────────────────────────
    const altRatio=Math.min(alt/AC.maxAlt,1);
    let sTop,sMid,sBot;
    if(isNight){sTop='#010306';sMid='#020508';sBot='#03070d';}
    else if(isDusk){sTop='#0d1a3a';sMid='#8b2500';sBot='#e84b10';}
    else{
      const dr=Math.round(30+altRatio*20), dg=Math.round(90+altRatio*50), db=Math.round(180+altRatio*60);
      sTop=`rgb(${dr},${dg},${db})`;sMid=`rgb(${80+altRatio*30},${150+altRatio*40},220)`;sBot='#b8ddff';
    }
    const sg=ctx.createLinearGradient(0,0,0,H*0.7);
    sg.addColorStop(0,sTop);sg.addColorStop(0.55,sMid);sg.addColorStop(1,sBot);
    ctx.fillStyle=sg;ctx.fillRect(0,0,W,H);

    // Sun/moon
    const sunA=(tod-0.25)*Math.PI*2;
    const sunX=W*0.5+Math.cos(sunA)*W*0.45;
    const sunY=H*0.45-Math.sin(sunA)*H*0.55;
    if(!isNight){
      const sr=ctx.createRadialGradient(sunX,sunY,0,sunX,sunY,isDusk?70:45);
      sr.addColorStop(0,isDusk?'#ffcc44':'#fffde0');
      sr.addColorStop(0.25,isDusk?'#ff880088':'#ffff6644');
      sr.addColorStop(1,'transparent');
      ctx.fillStyle=sr;ctx.fillRect(0,0,W,H);
      ctx.beginPath();ctx.arc(sunX,sunY,isDusk?20:16,0,Math.PI*2);
      ctx.fillStyle=isDusk?'#ffaa22':'#fffff0';ctx.fill();
    } else {
      // Moon + stars
      ctx.beginPath();ctx.arc(W*0.78,H*0.10,13,0,Math.PI*2);ctx.fillStyle='#d8d8bb';ctx.fill();
      for(let i=0;i<120;i++){
        const sx=((i*157+s.x*0.0008)%W+W)%W,sy=(i*61+3)%(H*0.55);
        const sB=0.25+Math.sin(i*0.7+s.t*1.8)*0.35;
    ...
    if(!isNight){
      const hg=ctx.createLinearGradient(0,H*0.48,0,H*0.7);
      hg.addColorStop(0,isDusk?'rgba(200,80,10,0.4)':'rgba(180,220,255,0.35)');
      hg.addColorStop(1,'transparent');
      ctx.fillStyle=hg;ctx.fillRect(0,H*0.48,W,H*0.25);
    }

    // Horizon Y — rises as altitude increases
    const horizY=H*(0.64-altRatio*0.30);

    // ── FAR MOUNTAINS ─────────────────────────────────────────────
    ctx.beginPath();ctx.moveTo(0,horizY);
    for(let i=0;i<=W;i+=18){
      const mx=i+((s.x*0.07)%200);
      ctx.lineTo(i,horizY-18-Math.sin(mx*0.014)*32-Math.cos(mx*0.006)*22);
    }
    ctx.lineTo(W,horizY);ctx.closePath();
    ctx.fillStyle=isNight?'#080f1a':'#1e3055';ctx.fill();

    // ── CLOUDS ─────────────────────────────────────────────────────
    s.clouds.forEach(c=>{
      const cy=horizY-(c.y*(H*0.45/500))-alt*0.12;
      if(cy<-80||cy>H*0.75) return;
      ctx.save();ctx.globalAlpha=isNight?Math.min(c.alpha*0.25,0.20):c.alpha;
      const cg=ctx.createRadialGradient(c.x+W/2,cy,0,c.x+W/2,cy,c.w*0.55);
      cg.addColorStop(0,isNight?'#1a2535':'#e8f0ff');
      cg.addColorStop(0.5,isNight?'#101820':'#d0e4ff');
      cg.addColorStop(1,'transparent');
      ctx.fillStyle=cg;
      ctx.beginPath();ctx.ellipse(c.x+W/2,cy,c.w*0.55,c.h*0.48,0,0,Math.PI*2);ctx.fill();
      ctx.restore();
    });

    // ── GROUND (perspective) ───────────────────────────────────────
    const gVis=Math.min(1,(1-alt/(AC.maxAlt*0.45))*2);
    if(gVis>0.02){
      ctx.save();ctx.globalAlpha=gVis;
      const gg=ctx.createLinearGradient(0,horizY,0,H);
      gg.addColorStop(0,isNight?'#0a120a':'#2a5218');
      gg.addColorStop(0.25,isNight?'#0f180a':'#3a6820');
      gg.addColorStop(1,isNight?'#080c05':'#1a3a0c');
      ctx.fillStyle=gg;ctx.fillRect(0,horizY,W,H-horizY);

      // Grid lines (runway/road perspective)
      if(!isNight&&alt<300){
        ctx.globalAlpha=gVis*(1-alt/300)*0.25;
        ctx.strokeStyle='#4a7a28';ctx.lineWidth=0.5;
        for(let gx=0;gx<W;gx+=60){ctx.beginPath();ctx.moveTo(gx,horizY);ctx.lineTo(gx+(gx-W/2)*0.3,H);ctx.stroke();}
        for(let gy=0;gy<8;gy++){const ry=horizY+(H-horizY)*(gy/8);ctx.beginPath();ctx.moveTo(0,ry);ctx.lineTo(W,ry);ctx.stroke();}
        ctx.globalAlpha=gVis;
      }

      // Terrain undulation
      ctx.beginPath();ctx.moveTo(0,horizY);
      s.terrain.forEach(t=>ctx.lineTo(t.x+W/2,horizY-t.h*(1-altRatio)*0.35));
      ctx.lineTo(W,horizY);ctx.fillStyle=isNight?'#0d1a08':'#2d5015';ctx.fill();

      // Trees
      s.trees.forEach(t=>{
        const tx=t.x+W/2;if(tx<-20||tx>W+20) return;
        const tS=Math.max(0.1,1-alt/AC.maxAlt*2.5);
        const th=t.h*tS;
        // Pine shape
        ctx.fillStyle=isNight?'#071007':'#1a380c';
        ctx.beginPath();ctx.moveTo(tx,horizY-th);ctx.lineTo(tx-6*tS,horizY);ctx.lineTo(tx+6*tS,horizY);ctx.closePath();ctx.fill();
        ctx.beginPath();ctx.moveTo(tx,horizY-th*0.7);ctx.lineTo(tx-9*tS,horizY-th*0.1);ctx.lineTo(tx+9*tS,horizY-th*0.1);ctx.closePath();
        ctx.fillStyle=isNight?'#091409':'#254a10';ctx.fill();
      });

      // Buildings with windows
      s.buildings.forEach(b=>{
        const bx=b.x+W/2;if(bx<-80||bx>W+80) return;
        const bS=Math.max(0.05,1-alt/AC.maxAlt*2);
        const bh=b.h*bS,bw=b.w*bS;
        // Building body
        const bGrad=ctx.createLinearGradient(bx-bw/2,horizY-bh,bx+bw/2,horizY);
        bGrad.addColorStop(0,isNight?'#1a2035':'#5a6e7a');
        bGrad.addColorStop(1,isNight?'#0d1520':'#384a55');
        ctx.fillStyle=bGrad;ctx.fillRect(bx-bw/2,horizY-bh,bw,bh);
        // Windows
        if(bS>0.2){
          const cols=Math.max(1,Math.floor(bw/8)),rows=Math.min(b.floors,Math.floor(bh/10));
          for(let wr=0;wr<rows;wr++) for(let wc=0;wc<cols;wc++){
            const lit=isNight?Math.random()>0.3:Math.random()>0.7;
            if(lit){ctx.fillStyle=isNight?`rgba(255,220,100,0.85)`:`rgba(180,200,220,0.6)`;
              ctx.fillRect(bx-bw/2+(wc+0.15)*bw/cols,horizY-bh+(wr+0.1)*bh/rows,bw/cols*0.65,bh/rows*0.7);}
          }
        }
        // Rooftop light
        if(isNight){ctx.fillStyle=`rgba(255,80,80,${0.5+Math.sin(s.t*3+b.x)*0.5})`;ctx.fillRect(bx-1,horizY-bh-3,2,3);}
      });

      // Helipad at x=0
      const padX=W/2-s.x*0.4;
      if(padX>-120&&padX<W+120){
        ctx.fillStyle='#444';ctx.fillRect(padX-50,horizY-1,100,6);
        // H marking
        ctx.fillStyle=isNight?'#ffff88':'#fff';
        ctx.font=`bold ${Math.max(8,12*Math.max(0.1,1-alt/500))}px Arial`;
        ctx.textAlign='center';ctx.fillText('H',padX,horizY+16);
        // Pad lights
        if(isNight||alt<200){
          const lc=`rgba(255,255,100,${0.7+Math.sin(s.t*4)*0.3})`;
          ctx.fillStyle=lc;
          [[-50,0],[50,0],[0,0]].forEach(([ox,oy])=>{ctx.beginPath();ctx.arc(padX+ox,horizY+oy,3,0,Math.PI*2);ctx.fill();});
        }
      }
      ctx.restore();
    }
    ...
    s.particles.forEach(p=>{
      const px=W/2+p.x,py=horizY+p.y;if(py<0||py>H) return;
      ctx.save();ctx.globalAlpha=p.life*0.5;
      ctx.beginPath();ctx.arc(px,py,2+p.life*5,0,Math.PI*2);
      ctx.fillStyle='#c0a060';ctx.fill();ctx.restore();
    });
    ...
    if(s.crashed){
      ...
      const fg=ctx.createRadialGradient(W/2,horizY,0,W/2,horizY,80);
      fg.addColorStop(0,'rgba(255,180,0,0.9)');fg.addColorStop(0.5,'rgba(255,60,0,0.5)');fg.addColorStop(1,'transparent');
      ctx.fillStyle=fg;ctx.fillRect(W/2-100,horizY-80,200,90);ctx.restore();
      s.crashParts.forEach(p=>{
        ctx.save();ctx.globalAlpha=Math.min(1,p.life);
        ctx.beginPath();ctx.arc(W/2+p.x,horizY-p.y,p.r,0,Math.PI*2);
        ctx.fillStyle=p.col;ctx.fill();ctx.restore();
      });
    }

    // ── AIRCRAFT — detailed eVTOL lift+cruise ─────────────────────
    const acX=W/2, acY=horizY-Math.min(alt*(H*0.52/AC.maxAlt),H*0.50)-12;
    const pitchR=s.pitch*Math.PI/180*0.5;
    const rollR=s.roll*Math.PI/180*0.35;
    const rpmA=Math.min(1,s.engineRPM/90);

    ctx.save();
    ctx.translate(acX,acY);
    ctx.rotate(-pitchR+rollR*0.3);

    // Rotor glow disks
    const rotorPositions=[[-36,-8],[-20,-8],[-4,-8],[10,-8],[26,-8],[40,-8]];
    rotorPositions.forEach(([rx,ry],ri)=>{
      ctx.save();ctx.translate(rx,ry);
      // Disk blur
      if(rpmA>0.05){
        const rdg=ctx.createRadialGradient(0,0,0,0,0,11);
        rdg.addColorStop(0,`rgba(120,180,255,${rpmA*0.40})`);
        rdg.addColorStop(1,'transparent');
        ctx.fillStyle=rdg;ctx.fillRect(-12,-12,24,24);
      }
      // Hub
      ctx.fillStyle=rpmA>0.1?'#1a2840':'#2a3a52';
      ctx.beginPath();ctx.arc(0,0,2.5,0,Math.PI*2);ctx.fill();
      // Spinning blades (3 per rotor)
      if(rpmA>0.08){
        for(let b=0;b<3;b++){
          ctx.save();ctx.rotate(s.rot[ri]+b*2.094);
          ctx.fillStyle=`rgba(140,165,195,${rpmA*0.75})`;
          ctx.beginPath();ctx.moveTo(-1,-1);ctx.lineTo(-1,-10);ctx.lineTo(1,-10);ctx.lineTo(1,-1);ctx.closePath();ctx.fill();
          ctx.restore();
        }
        // Blur ring
        ctx.strokeStyle=`rgba(160,200,240,${rpmA*0.25})`;ctx.lineWidth=1.5;
        ctx.beginPath();ctx.arc(0,0,9,0,Math.PI*2);ctx.stroke();
      }
      ctx.restore();
    });

    // ── Fuselage (teardrop/pod shape) ─────────────────────────────
    ctx.save();
    // Main body
    const bGrad=ctx.createLinearGradient(-24,0,24,0);
    bGrad.addColorStop(0,'#1c2d42');bGrad.addColorStop(0.5,'#2a3e58');bGrad.addColorStop(1,'#1a2a3c');
    ctx.fillStyle=bGrad;
    ctx.beginPath();
    ctx.moveTo(-24,0);
    ctx.bezierCurveTo(-30,-2,-30,8,-24,8);
    ctx.lineTo(18,8);
    ctx.bezierCurveTo(28,8,30,4,28,0);
    ctx.bezierCurveTo(30,-4,28,-6,18,-6);
    ctx.lineTo(-24,-6);
    ctx.bezierCurveTo(-30,-6,-30,-2,-24,0);
    ctx.closePath();ctx.fill();
    ctx.strokeStyle='#3a5570';ctx.lineWidth=0.6;ctx.stroke();

    // Cockpit glass bubble
    const cgGrad=ctx.createLinearGradient(-18,-6,0,4);
    cgGrad.addColorStop(0,'rgba(100,180,240,0.85)');
    cgGrad.addColorStop(0.4,'rgba(60,130,200,0.7)');
    cgGrad.addColorStop(1,'rgba(20,60,120,0.8)');
    ctx.fillStyle=cgGrad;
    ctx.beginPath();
    ctx.ellipse(-12,-2,9,6,0,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='rgba(180,220,255,0.5)';ctx.lineWidth=0.5;ctx.stroke();
    // Cockpit highlight
    ctx.fillStyle='rgba(220,240,255,0.35)';
    ctx.beginPath();ctx.ellipse(-15,-4,5,2.5,-0.3,0,Math.PI*2);ctx.fill();
    ctx.restore();

    // ── Main wing ─────────────────────────────────────────────────
    const wSpan=60, wSweep=6;
    const wGrad=ctx.createLinearGradient(-wSpan,0,wSpan,0);
    wGrad.addColorStop(0,'#1a2a3c');wGrad.addColorStop(0.5,'#2a3e56');wGrad.addColorStop(1,'#1a2a3c');
    ctx.fillStyle=wGrad;
    ctx.beginPath();
    ctx.moveTo(-wSpan,2);ctx.lineTo(-4-wSweep,-2);ctx.lineTo(14,-2);ctx.lineTo(wSpan+4,2);
    ctx.lineTo(wSpan+4,5);ctx.lineTo(14,5);ctx.lineTo(-4-wSweep,5);ctx.lineTo(-wSpan,5);
    ctx.closePath();ctx.fill();
    ctx.strokeStyle='#3a5570';ctx.lineWidth=0.5;ctx.stroke();

    // Wing leading edge highlight
    ctx.strokeStyle='rgba(100,160,220,0.3)';ctx.lineWidth=0.8;
    ctx.beginPath();ctx.moveTo(-wSpan,2);ctx.lineTo(-4-wSweep,-2);ctx.lineTo(14,-2);ctx.lineTo(wSpan+4,2);ctx.stroke();

    // Wing winglets
    [-1,1].forEach(side=>{
      ctx.save();ctx.translate(side*(wSpan+4),3.5);ctx.rotate(side*0.3);
      ctx.fillStyle='#253848';
      ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(side*3,-8);ctx.lineTo(side*5,-7);ctx.lineTo(side*2,0);ctx.closePath();ctx.fill();
      ctx.restore();
    });

    // ── V-Tail ────────────────────────────────────────────────────
    const vtAngle=45*Math.PI/180;
    [1,-1].forEach(side=>{
      ctx.fillStyle='#1e2e42';
      ctx.beginPath();
      ctx.moveTo(16,0);
      ctx.lineTo(26,side*-14);
      ctx.lineTo(28,side*-12);
      ctx.lineTo(20,side*1);
      ctx.closePath();ctx.fill();
      ctx.strokeStyle='#2a4060';ctx.lineWidth=0.5;ctx.stroke();
    });
    ...
    ctx.beginPath();ctx.moveTo(16,4);ctx.lineTo(24,10);ctx.lineTo(18,4);ctx.closePath();ctx.fill();

    // ── Nav lights ────────────────────────────────────────────────
    const blink=Math.sin(s.t*4)>0;
    ctx.beginPath();ctx.arc(-wSpan,3,2.5,0,Math.PI*2);
    ctx.fillStyle=`rgba(255,40,40,${blink?0.95:0.15})`;ctx.fill();
    ctx.beginPath();ctx.arc(wSpan+4,3,2.5,0,Math.PI*2);
    ctx.fillStyle=`rgba(40,255,40,${blink?0.95:0.15})`;ctx.fill();
    // Strobe
    if(Math.sin(s.t*7)>0.88){
    ...
    if(s.rainIntensity>0.05){
      ctx.save();ctx.globalAlpha=s.rainIntensity*0.38;
      const rainCount=Math.floor(s.rainIntensity*150);
      ctx.strokeStyle='#99ccff';ctx.lineWidth=0.6;
      for(let r=0;r<rainCount;r++){
        const rx=((r*181+s.t*140)%W+W)%W;
        const ry=((r*97+s.t*420)%H+H)%H;
    ...
    // ── GLASS COCKPIT HUD ─────────────────────────────────────────
    // PFD (Primary Flight Display) style
    const hudAlpha=0.95;
    ctx.save();ctx.globalAlpha=hudAlpha;

    // Attitude indicator (artificial horizon band)
    {
      const aiX=W/2,aiY=acY,aiR=70;
      ctx.save();ctx.beginPath();ctx.arc(aiX,aiY,aiR,0,Math.PI*2);ctx.clip();
      // Sky/ground split
      ctx.save();ctx.translate(aiX,aiY);ctx.rotate(-s.pitch*Math.PI/180*0.8);
      ctx.fillStyle='#1a4a8a';ctx.fillRect(-aiR,-aiR*2,aiR*2,aiR*2);
      ctx.fillStyle='#4a2a10';ctx.fillRect(-aiR,0,aiR*2,aiR*2);
      ctx.strokeStyle='rgba(255,220,0,0.9)';ctx.lineWidth=2;
      ctx.beginPath();ctx.moveTo(-aiR,0);ctx.lineTo(aiR,0);ctx.stroke();
      // Pitch ladder
      for(let pd=-20;pd<=20;pd+=5){
        if(pd===0) continue;
        const py=-pd*3;const len=pd%10===0?28:16;
        ctx.strokeStyle=`rgba(255,255,255,${pd%10===0?0.7:0.4})`;ctx.lineWidth=1;
        ctx.beginPath();ctx.moveTo(-len,py);ctx.lineTo(len,py);ctx.stroke();
        if(pd%10===0){ctx.fillStyle='rgba(255,255,255,0.7)';ctx.font='8px monospace';ctx.textAlign='right';ctx.fillText(pd,-len-3,py+3);}
      }
      ctx.restore();
      // Bank angle indicator arc
      ctx.save();ctx.translate(aiX,aiY);
      ctx.strokeStyle='rgba(255,220,50,0.6)';ctx.lineWidth=1.2;
      ctx.beginPath();ctx.arc(0,0,aiR-6,-Math.PI*0.75,-Math.PI*0.25);ctx.stroke();
      const bankDeg=Math.max(-60,Math.min(60,s.roll));
      ctx.rotate(-bankDeg*Math.PI/180);
      ctx.fillStyle='rgba(255,220,50,0.85)';
      ctx.beginPath();ctx.moveTo(0,-aiR+8);ctx.lineTo(-5,-aiR+18);ctx.lineTo(5,-aiR+18);ctx.closePath();ctx.fill();
      ctx.restore();
      ctx.restore();
      // AI border
      ctx.strokeStyle='rgba(80,120,180,0.8)';ctx.lineWidth=1.5;
      ctx.beginPath();ctx.arc(aiX,aiY,aiR,0,Math.PI*2);ctx.stroke();
      // Fixed crosshair
      ctx.strokeStyle='rgba(255,220,50,0.9)';ctx.lineWidth=2.5;
      ctx.beginPath();ctx.moveTo(aiX-32,aiY);ctx.lineTo(aiX-12,aiY);
      ctx.moveTo(aiX+12,aiY);ctx.lineTo(aiX+32,aiY);
      ctx.moveTo(aiX,aiY-8);ctx.lineTo(aiX,aiY-2);ctx.stroke();
      ctx.beginPath();ctx.arc(aiX,aiY,3.5,0,Math.PI*2);
      ctx.fillStyle='rgba(255,220,50,0.9)';ctx.fill();
    }

    // ── Altitude tape (right) ─────────────────────────────────────
    {const tx=W-62,ty=H/2;
    ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(tx,ty-90,58,180);
    ctx.strokeStyle='rgba(0,200,100,0.3)';ctx.lineWidth=0.5;ctx.strokeRect(tx,ty-90,58,180);
    for(let da=-5;da<=5;da++){
      const av=Math.round((alt+da*20)/20)*20,py2=ty-da*18;
      if(py2<ty-90||py2>ty+90) continue;
      ctx.strokeStyle='rgba(0,200,100,0.2)';ctx.beginPath();ctx.moveTo(tx,py2);ctx.lineTo(tx+7,py2);ctx.stroke();
      ctx.fillStyle='rgba(0,200,100,0.75)';ctx.font='9px "Courier New",monospace';
      ctx.textAlign='left';ctx.fillText(Math.max(0,av),tx+10,py2+3);
    }
    ctx.fillStyle='#00c864';ctx.fillRect(tx-2,ty-11,60,22);
    ctx.fillStyle='#000';ctx.font='bold 12px "Courier New",monospace';ctx.textAlign='center';
    ctx.fillText(alt.toFixed(0),tx+29,ty+4);
    ctx.fillStyle='rgba(0,200,100,0.6)';ctx.font='7px monospace';ctx.fillText('ALT m',tx+29,ty+80);}

    // ── Speed tape (left) ─────────────────────────────────────────
    {const tx=4,ty=H/2;
    ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(tx,ty-90,58,180);
    for(let ds=-5;ds<=5;ds++){
      const sv=Math.round((Math.abs(s.vx)+ds*5)/5)*5,py2=ty-ds*18;
      if(py2<ty-90||py2>ty+90) continue;
      ctx.strokeStyle='rgba(0,200,100,0.2)';ctx.lineWidth=0.5;ctx.beginPath();ctx.moveTo(tx+51,py2);ctx.lineTo(tx+58,py2);ctx.stroke();
      ctx.fillStyle='rgba(0,200,100,0.75)';ctx.font='9px "Courier New",monospace';
      ctx.textAlign='right';ctx.fillText(Math.max(0,sv),tx+50,py2+3);
    }
    ctx.fillStyle='#00c864';ctx.fillRect(tx,ty-11,60,22);
    ctx.fillStyle='#000';ctx.font='bold 12px "Courier New",monospace';ctx.textAlign='center';
    ctx.fillText(Math.abs(s.vx).toFixed(1),tx+30,ty+4);
    ctx.fillStyle='rgba(0,200,100,0.6)';ctx.font='7px monospace';ctx.fillText('IAS m/s',tx+30,ty+80);}

    // ── Heading tape (top center) ─────────────────────────────────
    {const hdg=((s.heading)%360+360)%360;
    ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(W/2-90,4,180,22);
    for(let dh=-5;dh<=5;dh++){
      const hx=W/2+dh*16;
      const hv=((Math.round(hdg/10+dh)*10)%360+360)%360;
      ctx.fillStyle=dh===0?'#00c864':'rgba(0,200,100,0.55)';
      ctx.font=dh===0?'bold 10px "Courier New",monospace':'9px "Courier New",monospace';
      ctx.textAlign='center';ctx.fillText(hv.toFixed(0)+'°',hx,20);
    }
    ctx.fillStyle='#00c864';ctx.fillRect(W/2-1,26,2,6);}

    // ── VSI ───────────────────────────────────────────────────────
    {const vsX=W-62,vsY=H*0.17;
    ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(vsX,vsY,58,70);
    ctx.strokeStyle='rgba(0,200,100,0.4)';ctx.lineWidth=0.5;ctx.strokeRect(vsX,vsY,58,70);
    const vsC=s.vy>0.5?'#00c864':s.vy<-1?'#ff4444':'#ffaa00';
    ctx.fillStyle='rgba(0,200,100,0.6)';ctx.font='7px monospace';ctx.textAlign='center';ctx.fillText('V/S',vsX+29,vsY+12);
    ctx.fillStyle=vsC;ctx.font='bold 12px "Courier New",monospace';ctx.textAlign='center';
    ctx.fillText((s.vy>=0?'+':'')+s.vy.toFixed(1),vsX+29,vsY+32);
    ctx.fillStyle='rgba(0,200,100,0.5)';ctx.font='7px monospace';ctx.fillText('m/s',vsX+29,vsY+45);
    const vB=Math.min(1,Math.max(-1,s.vy/12));
    ctx.fillStyle=vsC;
    if(vB>0) ctx.fillRect(vsX+10,vsY+55-20*vB,38,20*vB);
    else ctx.fillRect(vsX+10,vsY+55,38,-20*vB);}

    // ── Battery & Power arc gauges (bottom left cluster) ──────────
    {const gX=70,gY=H-70;
    // Battery arc
    const batPct=s.soc;const batC=batPct>0.3?'#00c864':batPct>0.15?'#ffaa00':'#ff2244';
    ctx.strokeStyle='rgba(0,0,0,0.5)';ctx.lineWidth=8;
    ctx.beginPath();ctx.arc(gX,gY,26,-2.35,0.79);ctx.stroke();
    ctx.strokeStyle=batC;ctx.lineWidth=6;
    ctx.beginPath();ctx.arc(gX,gY,26,-2.35,-2.35+3.14*batPct);ctx.stroke();
    ctx.fillStyle=batC;ctx.font='bold 11px monospace';ctx.textAlign='center';ctx.fillText((batPct*100).toFixed(0),gX,gY+3);
    ctx.fillStyle='rgba(0,200,100,0.5)';ctx.font='6.5px monospace';ctx.fillText('BAT%',gX,gY+15);

    // Power arc
    const pwrPct=Math.min(1,s.power/Math.max(AC.Phov,1));const pwrC=pwrPct>0.85?'#ff4444':pwrPct>0.6?'#ffaa00':'#00c864';
    ctx.strokeStyle='rgba(0,0,0,0.5)';ctx.lineWidth=8;
    ctx.beginPath();ctx.arc(gX+64,gY,26,-2.35,0.79);ctx.stroke();
    ctx.strokeStyle=pwrC;ctx.lineWidth=6;
    ctx.beginPath();ctx.arc(gX+64,gY,26,-2.35,-2.35+3.14*pwrPct);ctx.stroke();
    ctx.fillStyle=pwrC;ctx.font='bold 10px monospace';ctx.textAlign='center';ctx.fillText(s.power.toFixed(0),gX+64,gY+3);
    ctx.fillStyle='rgba(0,200,100,0.5)';ctx.font='6.5px monospace';ctx.fillText('kW',gX+64,gY+15);}

    // ── Phase + AP badge ──────────────────────────────────────────
    {const phC={GROUND:'#888',TAKEOFF:'#ffdd00',CLIMB:'#00c864',CRUISE:'#00ccff',DESCENT:'#ff8800',APPROACH:'#ffdd00',LEVEL:'#88ddff',LANDED:'#00c864',PREFLIGHT:'#555'}[s.phase]||'#aaa';
    ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(W/2-55,H-40,110,18);
    ctx.fillStyle=phC;ctx.font='bold 10px monospace';ctx.textAlign='center';ctx.fillText(s.phase,W/2,H-26);
    if(s.apMode!=='OFF'){ctx.fillStyle='rgba(0,40,160,0.85)';ctx.fillRect(W/2-42,H-60,84,18);ctx.fillStyle='#7ab4ff';ctx.font='bold 9px monospace';ctx.textAlign='center';ctx.fillText('AP: '+s.apMode,W/2,H-46);}}

    // ── Range/Time/Wind block ─────────────────────────────────────
    {ctx.fillStyle='rgba(0,0,0,0.6)';ctx.fillRect(4,4,118,54);
    ctx.strokeStyle='rgba(0,200,100,0.25)';ctx.lineWidth=0.5;ctx.strokeRect(4,4,118,54);
    ctx.fillStyle='rgba(0,200,100,0.8)';ctx.font='8.5px "Courier New",monospace';ctx.textAlign='left';
    ctx.fillText('RNG '+s.range.toFixed(1)+' km',10,18);
    ctx.fillText('T '+Math.floor(s.t/60)+':'+(s.t%60<10?'0':'')+Math.floor(s.t%60),10,32);
    ctx.fillStyle=s.gustX**2+s.gustY**2>4?'#ff8800':'rgba(0,200,100,0.6)';
    ctx.fillText('WND '+(Math.sqrt(s.windX**2+s.windY**2)).toFixed(1)+' m/s '+(s.gustX**2+s.gustY**2>4?'↯GUST':''),10,46);}

    // ── Warning overlay ───────────────────────────────────────────
    if(s.warning&&s.warnTimer>0){
      ctx.save();ctx.globalAlpha=0.65+Math.sin(s.t*9)*0.35;
      ctx.fillStyle='rgba(180,0,0,0.8)';ctx.fillRect(W/2-80,H*0.42,160,36);
      ctx.strokeStyle='#ff4444';ctx.lineWidth=1;ctx.strokeRect(W/2-80,H*0.42,160,36);
      ctx.fillStyle='#ffffff';ctx.font='bold 13px monospace';ctx.textAlign='center';
      ctx.fillText(s.warning,W/2,H*0.42+22);ctx.restore();
    }

    // ── SoC progress bar (bottom) ─────────────────────────────────
    {const bW=(W-130)*s.soc;const bC=s.soc>0.3?'#00c864':s.soc>0.15?'#ffaa00':'#ff2244';
    ctx.fillStyle='rgba(0,0,0,0.55)';ctx.fillRect(65,H-22,W-130,14);
    ctx.fillStyle=bC;ctx.fillRect(65,H-22,bW,14);
    ctx.fillStyle='rgba(255,255,255,0.8)';ctx.font='8px monospace';ctx.textAlign='left';
    ctx.fillText('BATTERY '+( s.soc*100).toFixed(0)+'% · '+((1-s.soc)*AC.packKwh).toFixed(1)+' kWh used',70,H-12);
    ctx.textAlign='right';ctx.fillText(AC.packKwh.toFixed(0)+' kWh pack',W-68,H-12);}
  ...
  const startSim=useCallback(()=>{
    const s=S.current;
    Object.assign(s,{x:0,y:0.1,vx:0,vy:0.1,soc:1,range:0,t:0,throttle:0.4,pitchCmd:0,rollCmd:0,pitch:0,roll:0,power:0,phase:'TAKEOFF',crashed:false,engineRPM:20,apMode:'OFF',apPitchI:0,apThrI:0,particles:[],crashParts:[],warning:'',warnTimer:0,rain:false,rainIntensity:0});
    lastT.current=null;
    setRunning(true);
  ...
  const cycleAP=useCallback(()=>{
    ...
    const nx=modes[(modes.indexOf(s.apMode)+1)%modes.length];
    s.apMode=nx;
    if(nx==='ALT') s.apAlt=Math.max(50,s.y);
    if(nx==='CRUISE'){s.apAlt=AC.maxAlt*0.9;s.apSpd=AC.vCr*0.85;}
    if(nx==='LND') s.apAlt=s.y;
    setDisp(d=>({...d,apMode:nx}));
  },[AC]);

  useEffect(()=>{
    const kd=(e)=>{ keys.current[e.key]=true;
      if((e.key==='p'||e.key==='P')&&running) cycleAP();
      if((e.key==='r'||e.key==='R')&&running){const s=S.current;s.rain=!s.rain;}
      if((e.key==='g'||e.key==='G')&&running){const s=S.current;s.gustX=s.gustX?0:14;s.gustY=s.gustY?0:-3;s.gustTimer=3;s.warning='⚡ GUST';s.warnTimer=2;}
      e.preventDefault?.();
    };
    const ku=(e)=>{delete keys.current[e.key];};
  ...
  return(<div style={{display:'flex',flexDirection:'column',gap:10}}>
    {/* Header */}
    <div style={{background:`linear-gradient(135deg,#020810,#0a1628)`,border:'1px solid #1a3a5c',borderRadius:10,padding:'12px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8}}>
      <div>
        <div style={{fontSize:8,color:'#3a6a9a',fontFamily:"'DM Mono',monospace",letterSpacing:'0.2em',marginBottom:2}}>X-PLANE STYLE · eVTOL FLIGHT SIMULATOR · 6-ROTOR LIFT+CRUISE</div>
        <div style={{fontSize:15,fontWeight:800,color:'#e2e8f0'}}>
          <span style={{color:'#3b82f6'}}>Trail 1 eVTOL</span> — MTOW {AC.MTOW.toFixed(0)} kg · {AC.Phov.toFixed(0)} kW hover · {AC.packKwh.toFixed(0)} kWh battery
        </div>
      </div>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        <button type="button" onClick={running?stopSim:startSim}
          style={{padding:'7px 22px',background:running?'rgba(220,0,0,0.2)':'rgba(0,180,80,0.2)',
            border:`1px solid ${running?'#dc2626':'#16a34a'}`,borderRadius:5,
            color:running?'#f87171':'#4ade80',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>
          {running?'⏹ STOP':'▶ TAKEOFF'}
        </button>
        {running&&(<>
          <button type="button" onClick={cycleAP}
            style={{padding:'7px 14px',background:(st.apMode&&st.apMode!=='OFF')?'rgba(30,80,200,0.4)':'transparent',
              border:`1px solid ${(st.apMode&&st.apMode!=='OFF')?'#3b82f6':'#334155'}`,borderRadius:5,
              color:(st.apMode&&st.apMode!=='OFF')?'#93c5fd':'#64748b',fontSize:10,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>
            AP: {st.apMode||'OFF'}
          </button>
          <button type="button" onClick={()=>{const s=S.current;s.rain=!s.rain;}}
            style={{padding:'7px 12px',background:(st.rain)?'rgba(30,80,180,0.3)':'transparent',
              border:`1px solid ${(st.rain)?'#60a5fa':'#334155'}`,borderRadius:5,
              color:(st.rain)?'#93c5fd':'#64748b',fontSize:10,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>
            🌧 Rain
          </button>
          <button type="button" onClick={()=>{const s=S.current;const on=s.gustX**2+s.gustY**2>1;s.gustX=on?0:14;s.gustY=on?0:-3;s.gustTimer=3;}}
            style={{padding:'7px 12px',background:st.gust?'rgba(180,80,0,0.3)':'transparent',
              border:`1px solid ${st.gust?'#f59e0b':'#334155'}`,borderRadius:5,
              color:st.gust?SC.amber:'#64748b',fontSize:10,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>
            ⚡ Gust
          </button>
        </>)}
      </div>
      <div style={{width:'100%',fontSize:9,color:'#3a6a9a',fontFamily:"'DM Mono',monospace"}}>{msg}</div>
    </div>

    {/* Canvas */}
    <div style={{borderRadius:8,overflow:'hidden',border:'1px solid #1a3a5c',background:'#010306'}}>
      <canvas ref={canvasRef} style={{display:'block',width:'100%',height:520}} height={520}/>
    </div>

    {/* Sliders */}
    {running&&(<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
      {[
        {label:'THROTTLE (W/S)',val:st.throttle||0,min:0,max:1,step:0.01,color:'#f59e0b',fmt:v=>(v*100).toFixed(0)+'%',cb:v=>{S.current.throttle=v;S.current.apMode='OFF';}},
        {label:'PITCH (A/D)',val:st.pitch||0,min:-20,max:20,step:0.5,color:'#14b8a6',fmt:v=>v.toFixed(1)+'°',cb:v=>{S.current.pitchCmd=v;S.current.apMode='OFF';}},
        {label:'ROLL (Q/E)',val:st.roll||0,min:-25,max:25,step:0.5,color:'#8b5cf6',fmt:v=>v.toFixed(1)+'°',cb:v=>{S.current.rollCmd=v;S.current.apMode='OFF';}},
      ].map(({label,val,min,max,step,color,fmt,cb})=>(
        <div key={label} style={{background:'#0d1117',border:'1px solid #1c2333',borderRadius:6,padding:'8px 12px'}}>
          <div style={{display:'flex',justifyContent:'space-between',fontSize:8,color:'#64748b',fontFamily:"'DM Mono',monospace",marginBottom:5}}>
            <span>{label}</span><span style={{color,fontWeight:700}}>{fmt(val)}</span>
          </div>
          <input type="range" min={min} max={max} step={step} value={val}
            onChange={e=>cb(+e.target.value)} style={{width:'100%'}}/>
        </div>
      ))}
    </div>)}
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:7}}>
      {[
        ...
        ['VERT SPD',(st.vy||0)>=0?'+'+(st.vy||0).toFixed(1):(st.vy||0).toFixed(1),'m/s',(st.vy||0)>=0?SC.green:SC.red],
        ['POWER',(st.power||0).toFixed(0),'kW',SC.amber],
        ['BATTERY',((st.soc||1)*100).toFixed(0),'%',socC],
        ['RANGE',(st.range||0).toFixed(1),'km',SC.blue],
      ].map(([l,v,u,c])=>(
        <div key={l} style={{background:'#0d1117',border:'1px solid #1c2333',borderRadius:6,padding:'8px 10px',textAlign:'center'}}>
          <div style={{fontSize:7,color:'#3a5a7a',fontFamily:"'DM Mono',monospace",marginBottom:1,letterSpacing:'0.08em'}}>{l}</div>
          <div style={{fontSize:18,fontWeight:700,color:c,fontFamily:"'DM Mono',monospace",lineHeight:1.1}}>{v}</div>
          <div style={{fontSize:7.5,color:'#3a5a7a',fontFamily:"'DM Mono',monospace",marginTop:1}}>{u}</div>
        </div>
      ))}
    </div>
    <div style={{fontSize:8,color:'#2a4a6a',fontFamily:"'DM Mono',monospace",padding:'5px 10px',background:'#020810',borderRadius:4,border:'1px solid #0d1f30'}}>
      CONTROLS: W/S Throttle · A/D Pitch · Q/E Roll · P Autopilot cycle (OFF→ALT HOLD→CRUISE→AUTO-LAND) · R Rain · G Gust &nbsp;|&nbsp;
      PHYSICS: SR-calibrated actuator disk · Dryden turbulence · Ground effect · Stall · Crash detection &nbsp;|&nbsp;
      V_stall={AC.Vstall.toFixed(1)} m/s · V_cr={AC.vCr} m/s · Phov={AC.Phov.toFixed(0)} kW · Pack={AC.packKwh.toFixed(0)} kWh
    </div>
  </div>);
}

/* ═══════════════════════════════════════════════════════════════
   FLEET OPTIMIZER — Multi-vertiport network economics
   ═══════════════════════════════════════════════════════════════ */
function FleetOptimizerPanel({ SR, params, SC, TTP }) {
  ...
  const [flightsPerDay,setFlightsPerDay]=useState(10);
  const [farePerKm,setFarePerKm]=useState(4.50);
  const [targetLF,setTargetLF]=useState(0.78);
  const [daysPerYear,setDaysPerYear]=useState(300);
  const [opMode,setOpMode]=useState('piloted');
  const [result,setResult]=useState(null);
  const activeRoutes=routes.filter(r=>r.active);
  const fmtM=(v)=>v>=1e9?`$${(v/1e9).toFixed(2)}B`:v>=1e6?`$${(v/1e6).toFixed(2)}M`:`$${(v/1000).toFixed(0)}k`;

  const runOptimize=useCallback(()=>{
    if(!SR||activeRoutes.length===0){setResult(null);return;}
    const routeResults=activeRoutes.map(r=>{
      const Efix=(SR.Eto||8)+(SR.Eld||4)+(SR.Eres||34);
      const Ecr_km=(SR.Ecr||92)/Math.max(params?.range||250,1);
      const Etot_r=Math.max(Efix+Ecr_km*r.dist,Efix*0.8);
      const flightTime_hr=r.dist/Math.max(params?.vCruise||67,1)/3600+((SR.tto||75)+(SR.tld||30)+(SR.tres||1200))/3600;
      const nSeats=Math.max(1,Math.round((params?.payload||455)/90));
      const flightsAC=Math.min(flightsPerDay,Math.floor(8/Math.max(flightTime_hr+0.25,0.1)));
      const paxPerFlight=nSeats*targetLF;
      const flightsNeeded=Math.ceil(r.demand/Math.max(paxPerFlight,0.1));
      const acNeeded=Math.ceil(flightsNeeded/Math.max(flightsAC,1));
      const CrHov=SR.CrateHov||3;
      const eta_d=Math.max(0.80,0.97-0.025*CrHov),eta_ch=Math.max(0.85,0.97-0.030*CrHov);
      const eCost=Etot_r/(eta_d*eta_ch)*0.16;
      const bCostKwh=(149*Math.pow(300/Math.max(100,params?.sedCell||300),0.3)+55)*2.0;
      const effCyc=Math.floor(Math.min(2000,900*Math.pow(0.5/Math.min(0.85,Etot_r/Math.max(SR.PackkWh||200,1)),0.6)*0.83));
      const battCost=(SR.PackkWh||200)*bCostKwh/Math.max(1,effCyc);
      const maintCost=45000/(flightsPerDay*daysPerYear)+(2.4*75+75)*flightTime_hr+1.48;
      const insCost=(SR.MTOW||2000)*800*0.10/(flightsPerDay*daysPerYear);
      ...
      const breakEvenLF=Math.min(1,docFlight/Math.max(nSeats*farePerKm*r.dist,0.01));
      return{...r,nSeats,flightTime_hr,flightsAC,flightsNeeded,acNeeded,Etot_r,docFlight,rev,profit,breakEvenLF,
        annualFlights:flightsNeeded*daysPerYear,annualRevenue:rev*flightsNeeded*daysPerYear,
        annualProfit:profit*flightsNeeded*daysPerYear,wh_km:Etot_r*1000/Math.max(r.dist,1)};
    });
    const totalAC=routeResults.reduce((s,r)=>s+r.acNeeded,0);
    const totalRev=routeResults.reduce((s,r)=>s+r.annualRevenue,0);
    const totalProfit=routeResults.reduce((s,r)=>s+r.annualProfit,0);
    const fleetAcq=totalAC*(SR.MTOW||2000)*800;
    const payback=totalProfit>0?fleetAcq/totalProfit:Infinity;
    const profitVsLF=Array.from({length:11},(_,i)=>{
      const lf=i/10;const obj={lf:+(lf*100).toFixed(0)};
      [{l:'Joby $1.86',f:1.86},{l:'NASA $4.50',f:4.50},{l:'Blade $6.00',f:6.00}].forEach(({l,f})=>{
        obj[l]=+routeResults.reduce((s,r)=>(s+(f*r.dist*r.nSeats*lf-r.docFlight)*r.flightsNeeded*daysPerYear),0).toFixed(0);
      });return obj;
    });
    setResult({routeResults,totalAC,totalRev,totalProfit,totalFlights:routeResults.reduce((s,r)=>s+r.annualFlights,0),fleetAcq,payback,profitVsLF});
  },[SR,params,activeRoutes,flightsPerDay,farePerKm,targetLF,daysPerYear,opMode]);

  useEffect(()=>{runOptimize();},[runOptimize]);
  ...
  return(<div style={{display:'flex',flexDirection:'column',gap:14}}>
    <div style={{background:`linear-gradient(135deg,${SC.bg},#1a0a2e)`,border:'1px solid #8b5cf644',borderRadius:10,padding:'16px 20px'}}>
      ...
      <div style={{fontSize:18,fontWeight:800,color:SC.text,marginBottom:4}}><span style={{color:'#a78bfa'}}>Fleet</span> Optimizer</div>
      <div style={{fontSize:11,color:SC.muted,lineHeight:1.7}}>Multi-route network economics. All DOC computed from your actual sizing outputs. Toggle routes, edit demand, adjust fare and operator mode.</div>
    </div>
    <div style={{display:'grid',gridTemplateColumns:'1.4fr 1fr',gap:14}}>
      <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14}}>
        <div style={{fontSize:10,fontWeight:700,color:SC.text,fontFamily:"'DM Mono',monospace",marginBottom:10,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          Route Network
          <button type="button" onClick={()=>{const id=Math.max(...routes.map(r=>r.id),0)+1;setRoutes(prev=>[...prev,{id,from:'Hub X',to:'Hub Y',dist:100,demand:80,active:true}]);}}
            style={{fontSize:9,padding:'3px 10px',background:`${SC.teal}22`,border:`1px solid ${SC.teal}44`,borderRadius:4,color:SC.teal,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>+ Add Route</button>
        </div>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:9,fontFamily:"'DM Mono',monospace"}}>
          <thead><tr style={{background:SC.bg}}>{['On','Route','Dist','Demand/day','✎'].map(h=><th key={h} style={{padding:'5px 6px',textAlign:'left',color:SC.muted,fontWeight:700,borderBottom:`1px solid ${SC.border}`}}>{h}</th>)}</tr></thead>
          <tbody>{routes.map(r=>(
            <tr key={r.id} style={{opacity:r.active?1:0.4}}>
              <td style={{padding:'5px 6px'}}><input type="checkbox" checked={r.active} onChange={()=>setRoutes(prev=>prev.map(x=>x.id===r.id?{...x,active:!x.active}:x))}/></td>
              <td style={{padding:'5px 6px',color:r.active?SC.text:SC.muted,fontWeight:r.active?700:400}}>{r.from}→{r.to}</td>
              <td style={{padding:'5px 6px',color:SC.amber}}>{r.dist} km</td>
              <td style={{padding:'5px 6px',color:SC.teal}}>{r.demand}</td>
              <td style={{padding:'5px 6px'}}><button type="button" onClick={()=>setEditRoute(r.id===editRoute?null:r.id)}
                style={{fontSize:8,padding:'2px 6px',background:`${SC.blue}22`,border:`1px solid ${SC.blue}44`,borderRadius:3,color:SC.blue,cursor:'pointer'}}>{r.id===editRoute?'✓':'✎'}</button></td>
            </tr>
          ))}</tbody>
        </table>
        {editRoute!=null&&(()=>{const r=routes.find(x=>x.id===editRoute);if(!r)return null;
          return(<div style={{marginTop:10,padding:'10px 12px',background:SC.bg,borderRadius:6,border:`1px solid ${SC.border}`,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {[['From',r.from,'from','text'],['To',r.to,'to','text'],['Distance (km)',r.dist,'dist','number'],['Daily demand (pax)',r.demand,'demand','number']].map(([lbl,val,k,type])=>(
              <div key={k}><div style={{fontSize:8,color:SC.muted,fontFamily:"'DM Mono',monospace",marginBottom:3}}>{lbl}</div>
              <input type={type} value={val} onChange={e=>setRoutes(prev=>prev.map(x=>x.id===editRoute?{...x,[k]:type==='number'?+e.target.value:e.target.value}:x))}
      ...
      <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14,display:'flex',flexDirection:'column',gap:10}}>
        ...
        {[{label:'Flights/aircraft/day',val:flightsPerDay,set:setFlightsPerDay,min:2,max:20,step:1,fmt:'0'},{label:'Fare ($/km)',val:farePerKm,set:setFarePerKm,min:1,max:12,step:0.1,fmt:'2'},{label:'Target load factor',val:targetLF,set:setTargetLF,min:0.3,max:1,step:0.01,fmt:'2'},{label:'Operating days/year',val:daysPerYear,set:setDaysPerYear,min:200,max:365,step:5,fmt:'0'},
        ].map(({label,val,set,min,max,step,fmt})=>(
          <div key={label} style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",minWidth:155}}>{label}</span>
            <input type="range" min={min} max={max} step={step} value={val} onChange={e=>set(+e.target.value)} style={{flex:1}}/>
            <span style={{fontSize:10,color:SC.amber,fontFamily:"'DM Mono',monospace",minWidth:40,textAlign:'right'}}>{val.toFixed(+fmt)}</span>
          </div>
        ))}
        <div><div style={{fontSize:9,color:SC.muted,fontFamily:"'DM Mono',monospace",marginBottom:6}}>Operator mode</div>
          <div style={{display:'flex',gap:6}}>
            {[['piloted','Piloted'],['remote','Remote'],['auto','Autonomous']].map(([m,lbl])=>(
              <button type="button" key={m} onClick={()=>setOpMode(m)}
                style={{flex:1,padding:'5px 0',background:opMode===m?'#8b5cf633':'transparent',border:`1px solid ${opMode===m?'#8b5cf6':SC.border}`,borderRadius:5,color:opMode===m?'#a78bfa':SC.muted,fontSize:9,cursor:'pointer',fontFamily:"'DM Mono',monospace"}}>{lbl}</button>
            ))}
          </div>
        </div>
        {!SR&&<div style={{fontSize:9,color:SC.amber,fontFamily:"'DM Mono',monospace",padding:'6px 8px',background:`${SC.amber}11`,borderRadius:4,border:`1px solid ${SC.amber}33`}}>⚠ Run sizing first.</div>}
      </div>
    </div>
    {result&&(<>
      <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10}}>
        {[['Fleet Size',result.totalAC+' aircraft',SC.amber],['Annual Revenue',fmtM(result.totalRev),SC.green],['Annual Profit',fmtM(result.totalProfit),result.totalProfit>0?SC.green:SC.red],['Fleet Acquisition',fmtM(result.fleetAcq),SC.blue],['Payback',result.payback===Infinity?'Not viable':`${Math.min(99,result.payback).toFixed(1)} yr`,result.payback<5?SC.green:result.payback<15?SC.amber:SC.red]].map(([l,v,c])=>(
      ...
      <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14}}>
        ...
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:9,fontFamily:"'DM Mono',monospace"}}>
            <thead><tr style={{background:SC.bg}}>{['Route','Dist','AC','Flights/yr','DOC/flt','Rev/flt','Profit/flt','BE LF','Wh/km','Status'].map(h=><th key={h} style={{padding:'6px 8px',textAlign:'right',color:SC.muted,fontWeight:700,borderBottom:`1px solid ${SC.border}`,whiteSpace:'nowrap'}}>{h}</th>)}</tr></thead>
            <tbody>{result.routeResults.map((r,i)=>{const sc2=r.profit>0?SC.green:r.breakEvenLF<0.9?SC.amber:SC.red;return(
              <tr key={r.id} style={{background:i%2===0?SC.bg+'44':'transparent'}}>
                <td style={{padding:'6px 8px',textAlign:'left',color:SC.text,fontWeight:700}}>{r.from}→{r.to}</td>
                <td style={{padding:'6px 8px',textAlign:'right',color:SC.muted}}>{r.dist}</td>
                <td style={{padding:'6px 8px',textAlign:'right',color:SC.amber,fontWeight:700}}>{r.acNeeded}</td>
                ...
                <td style={{padding:'6px 8px',textAlign:'right',color:sc2,fontWeight:700}}>{r.profit>0?'✓ Profit':r.breakEvenLF<0.9?'~ Marginal':'✗ Loss'}</td>
              </tr>);
            })}</tbody>
          </table>
        </div>
      ...
      <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:8,padding:14}}>
        <div style={{fontSize:10,fontWeight:700,color:SC.text,fontFamily:"'DM Mono',monospace",marginBottom:8}}>Fleet Annual Profit vs Load Factor — Fare Sensitivity</div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={result.profitVsLF} margin={{top:5,right:20,left:10,bottom:20}}>
            ...
            <Line type="monotone" dataKey="Blade $6.00" stroke="#a78bfa" strokeWidth={2} dot={false}/>
          </LineChart>
        </ResponsiveContainer>
      </div>
    </>)}
                  ...
                  <Panel title="Range-Payload Diagram" ht={310}>
                    <ResponsiveContainer width="100%" height={260}>
                      <AreaChart data={SR.rpData} margin={{top:10,right:20,left:10,bottom:20}}>
                        ...
                        <Tooltip {...TTP}/>
                        <ReferenceLine x={params.payload} stroke={SC.amber} strokeDasharray="4 3" label={{value:"Design",fill:SC.amber,fontSize:11}}/>
                        <ReferenceLine y={params.range} stroke={SC.amber} strokeDasharray="4 3"/>
                ...
                <div style={{background:SC.panel,
                  ...
                  <div>
                    ...
                    <div style={{fontSize:10,color:SC.muted,marginTop:2,fontFamily:"'DM Mono',monospace"}}>
                      Generates a .vspscript that builds fuselage · wing · V-tail · {params.nPropHover} hover rotors · CG & NP  inside your OpenVSP
                    </div>
                  </div>
