...
function runSizing(p) {
  ...
  // 1 point per kg, low→high so AreaChart tooltip resolves correctly by index
  const rpData=Array.from({length:Math.ceil(maxPayload)+1},(_,i)=>{
    const pay=i;
        ...
        <div style={{position:"fixed",inset:0,zIndex:3000,background:"rgba(0,0,0,0.7)",
          ...
          <div style={{background:SC.panel,border:`1px solid ${SC.border}`,borderRadius:12,
            ...
            <div style={{marginTop:8,padding:"8px 12px",borderRadius:6,fontSize:9,
              ...
              {!user && "⚠️ Not logged in — branding saved locally only. Log in to sync across all devices."}
              {user && !brandSynced && "⏳ Syncing branding to your account..."}
              {user && brandSynced && "☁️ Synced to your account — logo & branding load on any device when you log in."}
            </div>
                    ...
                    <AreaChart
                      ...
                      <Tooltip
                        ...
                        formatter={(v,n,props)=>{
                          ...
                          const col=seg==="A"?SC.red:seg==="design"?SC.amber:"#a78bfa";
                          return[<span style={{color:col,fontWeight:700}}>{v} km</span>,"Range"];
                        }}
                        labelFormatter={(idx,payload)=>{
                          const kg=payload?.[0]?.payload?.payload??idx;
                          return `Payload: ${kg} kg`;
                        }}/>
                      ...
                      <ReferenceLine x={params.payload} stroke="none"
                        label={{
                          value:`● ${SR?SR.totalRange:params.range} km`,
