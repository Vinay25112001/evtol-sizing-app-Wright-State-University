...
function generateReport(p, SR, branding={}) {
  ...
  ${eq("W_E = \\frac{E_{total}\\times 1000}{(1-\\text{SoC}_{min})\\,\\text{SED}_{eff}\\,\\eta_{bat}} = \\frac{"+fmt(SR.Etot,3)+"\\times 1000}{(1-"+p.socMin+")\\times"+p.sedCell+"\\times "+p.etaBat+"} = "+fmt(SR.Wbat,2)+"\\text{ kg},\\quad W_P=\\frac{P_{hov}}{SP_{bat}}="+fmt(SR.Phov,1)+"\\,\\text{kW},\\;W_{bat}=\\max(W_E,W_P)","Dual-constraint battery mass")}
