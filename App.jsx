...
/* ═══════════════════════════════════════════════════════════════════════
   OPENVSP PYTHON SCRIPT GENERATOR
   Generates a Python script using the official OpenVSP Python API.
   The user runs:  python Trail1_eVTOL_builder.py
   This creates Trail1_eVTOL.vsp3 guaranteed valid for any OpenVSP version.
   ═══════════════════════════════════════════════════════════════════════ */
...
"""
Trail 1 eVTOL — OpenVSP Model Builder
Generated by eVTOL Sizer v2.0 (Wright State University)

HOW TO RUN:
  python Trail1_eVTOL_builder.py

REQUIREMENTS:
  OpenVSP must be installed and its Python bindings available.
  In OpenVSP 3.48.2, the Python API module is 'openvsp'.
  If you get an ImportError, add the OpenVSP python folder to your path:
    Windows:  set PYTHONPATH=C:\\\\OpenVSP-3.48.2-win64\\\\python
    Mac/Linux: export PYTHONPATH=/Applications/OpenVSP.app/.../python

OUTPUT: Trail1_eVTOL_tilt${tiltAngle}deg_builder.py  (opens directly in OpenVSP)

Design parameters:
  MTOW       = ${f(R.MTOW)} kg
  Wing span  = ${f(R.bWing)} m
  Wing area  = ${f(R.Swing)} m2
  Fuselage   = ${f(fL)} m long, ${f(fD)} m diameter
  Rotor diam = ${f(R.Drotor)} m
  CG         = ${f(R.xCGtotal)} m from nose
  NP         = ${f(R.xNP)} m from nose
  Static margin = ${f(R.SM*100,1)}% MAC
...
try:
    ...
    try:
        ...
        print("ERROR: Cannot import openvsp module.")
        print("Make sure OpenVSP Python bindings are on your PYTHONPATH.")
        print("See https://openvsp.org/api_docs/latest/index.html")
        raise

import sys, os

print("Trail 1 eVTOL — Building OpenVSP model...")

# ─── clear any existing model ────────────────────────────────────────────────
vsp.ClearVSPModel()
vsp.Update()

# ─── helper: set a parm by geom_id, parm_name, group_name, value ─────────────
def SP(gid, pname, gname, val):
    pid = vsp.GetParm(gid, pname, gname)
    if pid:
        vsp.SetParmVal(pid, float(val))
    else:
        print(f"  WARNING: Parm not found: {pname}/{gname} on {vsp.GetGeomName(gid)}")

def SPi(gid, pname, gname, val):
    pid = vsp.GetParm(gid, pname, gname)
    if pid:
        vsp.SetParmVal(pid, int(val))
    else:
        print(f"  WARNING: Parm not found: {pname}/{gname} on {vsp.GetGeomName(gid)}")
...
fus_id = vsp.AddGeom("FUSELAGE_GEOM")
vsp.SetGeomName(fus_id, "Fuselage")
SP(fus_id, "Length", "Design", ${f(fL)})

# Resize to 9 cross-sections
xsec_surf = vsp.GetXSecSurf(fus_id, 0)
# Default fuselage has 6 xsecs, we want 9
while vsp.GetNumXSec(xsec_surf) < 9:
    vsp.AppendXSec(xsec_surf, vsp.XS_SUPER_ELLIPSE)
while vsp.GetNumXSec(xsec_surf) > 9:
    vsp.CutXSec(xsec_surf, vsp.GetNumXSec(xsec_surf) - 1)

# Cross-section definitions: [x_frac, width, height]
xsec_data = [
    (0.000, 0.001,          0.001         ),  # 0 nose point
    (0.030, ${f(fD*0.38)},  ${f(fD*0.38)} ),  # 1
    (0.090, ${f(fD*0.90)},  ${f(fD*0.90)} ),  # 2
    (0.250, ${f(fD)},       ${f(fD)}      ),  # 3 max cabin
    (0.480, ${f(fD)},       ${f(fD)}      ),  # 4
    (0.650, ${f(fD*0.98)},  ${f(fD*0.95)} ),  # 5
    (0.800, ${f(fD*0.80)},  ${f(fD*0.75)} ),  # 6
    (0.920, ${f(fD*0.48)},  ${f(fD*0.38)} ),  # 7
    (1.000, ${f(fD*0.16)},  ${f(fD*0.10)} ),  # 8 tail
]
for i, (xfrac, w, h) in enumerate(xsec_data):
    xs = vsp.GetXSec(xsec_surf, i)
    vsp.SetXSecTanStrengths(xs, vsp.XSEC_BOTH_SIDES, 0.5, 0.5)
    vsp.ChangeXSecShape(xsec_surf, i, vsp.XS_SUPER_ELLIPSE if w > 0.01 else vsp.XS_POINT)
    xpid = vsp.GetParm(xs, "XLocPercent", "XSec")
    if xpid: vsp.SetParmVal(xpid, xfrac)
    if w > 0.01:
        wpid = vsp.GetParm(xs, "Super_Width", "SuperEllipse")
        hpid = vsp.GetParm(xs, "Super_Height", "SuperEllipse")
        if wpid: vsp.SetParmVal(wpid, w)
        if hpid: vsp.SetParmVal(hpid, h)

vsp.Update()
print(f"  Fuselage OK — {${f(fL)}} m long")

# ════════════════════════════════════════════════════════
# 2. MAIN WING
# ════════════════════════════════════════════════════════
print("  Building main wing...")
wing_id = vsp.AddGeom("WING_GEOM", fus_id)
vsp.SetGeomName(wing_id, "MainWing")

# XZ symmetry (left+right)
SPi(wing_id, "SymPlanFlag", "Sym", ${2})

# Position: root LE at xWingLE, z = zWing
SP(wing_id, "X_Rel_Location", "XForm", ${f(xWingLE)})
SP(wing_id, "Y_Rel_Location", "XForm", 0.0)
SP(wing_id, "Z_Rel_Location", "XForm", ${f(zWing)})

# Wing parameters
SP(wing_id, "TotalSpan",  "WingGeom", ${f(R.bWing)})
SP(wing_id, "TotalArea",  "WingGeom", ${f(R.Swing)})
SP(wing_id, "TotalAR",    "WingGeom", ${f(p.AR)})
SP(wing_id, "TotalTaper", "WingGeom", ${f(p.taper)})
SP(wing_id, "TotalSweep", "WingGeom", ${f(R.sweep)})
SP(wing_id, "SweepLoc",   "WingGeom", 0.0)
SP(wing_id, "Dihedral",   "WingGeom", 2.0)

# Wing section airfoil (NACA 4-series)
wing_surf = vsp.GetXSecSurf(wing_id, 0)
xs0 = vsp.GetXSec(wing_surf, 0)
vsp.ChangeXSecShape(wing_surf, 0, vsp.XS_FOUR_SERIES)
tc_pid = vsp.GetParm(xs0, "ThickChord", "FourSeries")
if tc_pid: vsp.SetParmVal(tc_pid, ${f(p.tc)})

vsp.Update()
print(f"  Main wing OK — span={${f(R.bWing)}} m  area={${f(R.Swing)}} m2")

# ════════════════════════════════════════════════════════
# 3. V-TAIL (both panels via XZ symmetry)
# ════════════════════════════════════════════════════════
print("  Building V-tail...")
vtail_id = vsp.AddGeom("WING_GEOM", fus_id)
vsp.SetGeomName(vtail_id, "VTail")

SPi(vtail_id, "SymPlanFlag", "Sym", ${2})
SP(vtail_id, "X_Rel_Location", "XForm", ${f(xVtLE)})
SP(vtail_id, "Y_Rel_Location", "XForm", 0.0)
SP(vtail_id, "Z_Rel_Location", "XForm", ${f(zVtail)})

SP(vtail_id, "TotalSpan",  "WingGeom", ${f(R.bvt_panel*2)})
SP(vtail_id, "TotalArea",  "WingGeom", ${f(R.Svt_total)})
SP(vtail_id, "TotalAR",    "WingGeom", ${f(p.vtAR)})
SP(vtail_id, "TotalTaper", "WingGeom", 0.40)
SP(vtail_id, "TotalSweep", "WingGeom", ${f(R.sweep_vt)})
SP(vtail_id, "SweepLoc",   "WingGeom", 0.0)
SP(vtail_id, "Dihedral",   "WingGeom", ${f(p.vtGamma)})

vsp.Update()
print(f"  V-tail OK — dihedral={${p.vtGamma}}deg  area={${f(R.Svt_total)}} m2")

# ════════════════════════════════════════════════════════
# 4. HELPER: add a PROP_GEOM
# ════════════════════════════════════════════════════════
def add_prop(name, parent_id, x, y, z, ry, diameter, n_blades, chord):
    pid = vsp.AddGeom("PROP_GEOM", parent_id)
    vsp.SetGeomName(pid, name)
    SP(pid,  "X_Rel_Location", "XForm", x)
    SP(pid,  "Y_Rel_Location", "XForm", y)
    SP(pid,  "Z_Rel_Location", "XForm", z)
    SP(pid,  "X_Rel_Rotation", "XForm", 0.0)
    SP(pid,  "Y_Rel_Rotation", "XForm", ry)
    SP(pid,  "Z_Rel_Rotation", "XForm", 0.0)
    SP(pid,  "Diameter",       "Design", diameter)
    SPi(pid, "Nblade",         "Design", n_blades)
    SP(pid,  "Beta34",         "Design", 22.0)
    SP(pid,  "HubDiameter",    "Design", diameter * 0.12)
    # Set blade chord via section (section 1 = mid-span)
    vsp.Update()
    return pid

# ════════════════════════════════════════════════════════
# 5. WINGTIP TILT ROTORS (1 per tip, tiltAngle from user)
#    Y_Rel_Rotation:  0  = thrust forward (cruise)
#                    -90 = thrust upward  (hover)
# ════════════════════════════════════════════════════════
print("  Adding wingtip tilt rotors (tilt=${tiltAngle}deg)...")
tilt_ry = -${tiltAngle}

wtr_port = add_prop("TiltRotor_PORT", fus_id,
    ${f(xtilt)}, +${f(ytip)}, ${f(ztilt)}, tilt_ry,
    ${f(R.Drotor)}, ${nb}, ${f(R.ChordBl)})

wtr_stbd = add_prop("TiltRotor_STBD", fus_id,
    ${f(xtilt)}, -${f(ytip)}, ${f(ztilt)}, tilt_ry,
    ${f(R.Drotor)}, ${nb}, ${f(R.ChordBl)})

print(f"  Tilt rotors OK — D={${f(R.Drotor)}} m  y=+-{${f(ytip)}} m  ry={tilt_ry}deg")

# ════════════════════════════════════════════════════════
# 6. BOOM LIFT ROTORS (2 per side, fore+aft at mid-wing Y)
#    Y position = bWing * 0.28  (mid-wing, from sizing calc)
#    Always vertical: Y_Rel_Rotation = -90
# ════════════════════════════════════════════════════════
print("  Adding boom lift rotors...")

boom_rots = [
    ("BoomRotor_PORT_FWD", +${f(yBoom)}, ${f(xBoomF)}),
    ("BoomRotor_PORT_AFT", +${f(yBoom)}, ${f(xBoomA)}),
    ("BoomRotor_STBD_FWD", -${f(yBoom)}, ${f(xBoomF)}),
    ("BoomRotor_STBD_AFT", -${f(yBoom)}, ${f(xBoomA)}),
]
for (rname, ry_val, rx_val) in boom_rots:
    add_prop(rname, fus_id,
        rx_val, ry_val, ${f(zBoom)}, -90.0,
        ${f(fxDiam)}, ${nb}, ${f(R.ChordBl*0.85)})

print(f"  Boom rotors OK — D={${f(fxDiam)}} m  y=+-{${f(yBoom)}} m (={${f(R.bWing*0.28/R.bWing*100,1)}}% semi-span)")

# ════════════════════════════════════════════════════════
# 7. TAIL PUSHER (horizontal, Y_Rel_Rotation = 0)
# ════════════════════════════════════════════════════════
print("  Adding tail pusher...")
pusher = add_prop("Tail_Pusher", fus_id,
    ${f(xPush)}, 0.0, ${f(fD*0.05)}, 0.0,
    ${f(pushD)}, 3, ${f(R.ChordBl*0.80)})
print(f"  Tail pusher OK — D={${f(pushD)}} m  x={${f(xPush)}} m")

# ════════════════════════════════════════════════════════
# 8. CG AND NP MARKERS (small FUSELAGE_GEOM spheroids)
# ════════════════════════════════════════════════════════
print("  Adding CG and NP markers...")
for mname, mx, mz in [("CG_Marker", ${f(R.xCGtotal)}, ${f(fD*0.55)}),
                      ("NP_Marker", ${f(R.xNP)},       ${f(fD*0.65)})]:
    mid = vsp.AddGeom("FUSELAGE_GEOM", fus_id)
    vsp.SetGeomName(mid, mname)
    SP(mid, "X_Rel_Location", "XForm", mx - 0.06)
    SP(mid, "Z_Rel_Location", "XForm", mz)
    SP(mid, "Length", "Design", 0.12)
    msurf = vsp.GetXSecSurf(mid, 0)
    for xi in range(vsp.GetNumXSec(msurf)):
        xs = vsp.GetXSec(msurf, xi)
        vsp.ChangeXSecShape(msurf, xi, vsp.XS_SUPER_ELLIPSE)
        d = 0.08 if xi in (0, vsp.GetNumXSec(msurf)-1) else 0.14
        for pn in ("Super_Width","Super_Height"):
            pp = vsp.GetParm(xs, pn, "SuperEllipse")
            if pp: vsp.SetParmVal(pp, d)
    vsp.Update()

# ════════════════════════════════════════════════════════
# 9. VEHICLE MASS PROPERTIES
# ════════════════════════════════════════════════════════
vid = vsp.FindGeom("Vehicle", 0) if hasattr(vsp, 'FindGeom') else None
if vid:
    SP(vid, "Mass", "Mass_Prop", ${f(R.MTOW)})
    SP(vid, "CGx",  "Mass_Prop", ${f(R.xCGtotal)})
    SP(vid, "CGy",  "Mass_Prop", 0.0)
    SP(vid, "CGz",  "Mass_Prop", 0.0)
...
# 10. SAVE
# ════════════════════════════════════════════════════════
outfile = "Trail1_eVTOL_tilt${tiltAngle}deg.vsp3"
vsp.WriteVSPFile(outfile, vsp.SET_ALL)
print()
print("=" * 60)
print(f"SUCCESS: {outfile}")
print("=" * 60)
print(f"  MTOW          = ${f(R.MTOW)} kg")
print(f"  Wing span     = ${f(R.bWing)} m")
print(f"  Wing area     = ${f(R.Swing)} m2")
print(f"  Rotor diam    = ${f(R.Drotor)} m")
print(f"  Boom rotor y  = +-${f(yBoom)} m from CL")
print(f"  Tilt angle    = ${tiltAngle} deg")
print(f"  CG            = ${f(R.xCGtotal)} m from nose")
print(f"  NP            = ${f(R.xNP)} m from nose")
print(f"  Static margin = ${f(R.SM*100,1)} % MAC")
print()
print(f"Open {outfile} in OpenVSP 3.48.2 to view the model.")
`;
}
            ...
            {tab===9&&(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{background:"linear-gradient(135deg,#0d1117 0%,#0f172a 100%)",
                  ...
                  <div>
                    <div style={{fontSize:9,color:C.muted,fontFamily:"'DM Mono',monospace",letterSpacing:"0.15em",marginBottom:4}}>GEOMETRY EXPORT — OpenVSP Python API</div>
                    <div style={{fontSize:22,fontWeight:800,color:C.amber,letterSpacing:"-0.03em"}}>Trail 1 eVTOL Python Builder</div>
                    <div style={{fontSize:10,color:C.muted,marginTop:2,fontFamily:"'DM Mono',monospace"}}>
                      Downloads a <b style={{color:C.amber}}>.py script</b> — run it with your OpenVSP Python to generate a guaranteed-valid .vsp3 file
                    </div>
                    <div style={{marginTop:6,padding:"5px 10px",background:"#0a0d14",borderRadius:4,
                      ...
                      python Trail1_eVTOL_builder.py  →  creates Trail1_eVTOL_tilt{tiltAngle}deg.vsp3
                    </div>
                  </div>
                  {/* Tilt angle control */}
                  ...
                  <div style={{marginLeft:"auto",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                    <button
                      onClick={()=>{
                        ...
                        a.href=url; a.download=`Trail1_eVTOL_tilt${tiltAngle}deg_builder.py`; a.click();
                      ...
                      ⬇  Download Python Builder ({tiltAngle}°)
                    </button>
                  </div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                  ...
                  <Panel title="How to Generate the .vsp3 (Python API method — works 100%)">
                    {[
                      ["1","Download Script","Click any download button to get a .py file with all your sizing values baked in."],
                      ["2","Set PYTHONPATH","Point Python to OpenVSP's python folder: e.g. set PYTHONPATH=C:\\OpenVSP-3.48.2-win64\\python (Windows) or export PYTHONPATH=/path/to/OpenVSP/python (Mac/Linux)."],
                      ["3","Run the Script","Open a terminal in the same folder: python Trail1_eVTOL_tiltXXdeg_builder.py"],
                      ["4","Open in OpenVSP","The script creates Trail1_eVTOL_tiltXXdeg.vsp3 — open it with File → Open in OpenVSP 3.48.2."],
                      ["5","Tilt Rotors","Set the slider (0°=cruise, 90°=hover) then download a new script to get a different rotor orientation."],
                      ["6","Iterate","Change any parameter in the sidebar → download a fresh script → re-run → new .vsp3. The geometry always reflects your latest calculations."],
                    ].map(([n,title,text])=>(
                ...
                <div style={{display:"flex",justifyContent:"center",gap:12,paddingTop:4,paddingBottom:8,flexWrap:"wrap"}}>
                  <button
                    onClick={()=>{
                      ...
                      const url=URL.createObjectURL(blob);
                      const a=document.createElement("a");
                      a.href=url; a.download="Trail1_eVTOL_CRUISE_0deg_builder.py"; a.click();
                      URL.revokeObjectURL(url);
                    }}
                  ...
                  <button
                    onClick={()=>{
                      ...
                      const url=URL.createObjectURL(blob);
                      const a=document.createElement("a");
                      a.href=url; a.download="Trail1_eVTOL_HOVER_90deg_builder.py"; a.click();
                      URL.revokeObjectURL(url);
                    }}
                    style={{padding:"10px 24px",background:"transparent",
